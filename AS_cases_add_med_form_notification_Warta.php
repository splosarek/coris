...
require_once('include/pdf_utils.php');
		...
		$kosztyc = $_POST['kosztyC'];
		$kosztyw = $_POST['kosztyW'];
		$okolicznosci = $row['circumstances'];
		$diagnoza = $row['event'];
		$polisa = $row['policy'];
		
		$uwagi = '&nbsp;'.nl2br($_POST['txtUwagi']);

		$template = '../fax/templates/med_form_notification_Warta.html';
		...
		if ($tmpl <> null){
 				...
 				'<!--KOSZTYC-->' => $kosztyc,'<!--KOSZTYW-->' => $kosztyw ,'<!--UWAGI-->' => $uwagi, '<!--STOPKA_FIRM-->' =>  STOPKA_FIRM
 				);
 		...
 		if (file_exists($file)){
				if (isset($_POST['send_fax']) || isset($_POST['send_email'])){ 
					if (isset($_POST['send_fax'])){		
    	        		...
    	        		$id = wysylka_fax($dir, basename($new_file),$nr );
            			if ($id>0)
         					sendFax($id,'', $message,  $nr);         					
       					...
       					if (strpos($email_to,";"))	
							...
							$tmpl = load_template('../fax/templates/email_template.html');
      						...
      						if ($tmpl <> null){   	
					      		$id = wysylka_email($dir,$email_to,$email_cc,$email_temat,$email_body);      	
					      		if ($id>0){
					      		 		sendEmail($id,$email_to,$email_cc,$email_temat,$email_body,$file_attach );
					      		}
      						}
       				}         				
   			 	}else{
...
function wysylka_fax($dir,$file,$nr){

  $query = "INSERT INTO coris_fax_out (ID,ID_section,`date`,ID_user,user_send,`dir`,`file`,number,ID_case,ID_contrahents,company,`to`) ";
  $query .= " values (null,1,now(),'".$_SESSION['user_id']."','".$_POST['editor']."','$dir','$file','$nr','".$_POST['case_id']."','".$_POST['contrahent_id']."','".$_POST['contrahent_name']."','".$_POST['contrahent_to_name']."')";
  $result = mysql_query($query);
  if ($result){
    $fid =  mysql_insert_id();
    ...
    $query .= " VALUES(null,'".$_POST['case_id']."',1,7,'".$_SESSION['user_id']."',2,'".$_POST['paxname']."','$nr','".$_POST['paxname']."','',now(),'$fid')";    
    $result = mysql_query($query);
...
function wysylka_email($dir,$email_to,$email_cc,$email_temat,$email_body){

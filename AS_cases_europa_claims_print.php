...
include_once('lib/lib_europa.php');
  ...
  if ($id>0) {
      ...
      $query = "SELECT * FROM coris_europa_decisions  WHERE ID='$id'";
      ...
      $row_case_ann = $europa_case->getCaseeuropaInfo($case_id);
 ...
 if ($row['type'] == 3){ //akceptacja        
      $page_template->load_template('../fax/templates/europa_decyzja_tak.html');
...
//      $page_template->set('<!--TEXT_1-->' , getTxt1($row['payment_amount']) );
      ...
      $page_template->load_template('../fax/templates/europa_decyzje_operat.html');
     ...
     // 	$body .=  $page_template->realize();
      ...
      $page_template->load_template('../fax/templates/europa_decyzja_nie.html');
      ...
      $page_template->load_template('../fax/templates/europa_decyzje_operat_odmowa.html');
      ...
      $page_template->set('<!--KONTO_NR-->' , '' );
      	...
      	$page_template->set('<!--SUMY_UBEZPIECZ-->' , stripslashes($row['text3']) );
      	$page_template->set('<!--WYPLATA-->' , '0 PLN');
  		...
  		$tmp = file("../fax/templates/europan_decyzja_header.html");
  		$header = implode('', $tmp);
  		$tmp = file("../fax/templates/europa_decyzja_footer.html");
  		$footer = implode('', $tmp);
   ...
   if ($body <> null){
		...
		$margins =  array('left' => 0,'right'  => 0,'top'    =>30,'bottom' => 30); 
...
<span style="font-size:8px;">S±d Rejonowy dla Wroc³awia Fabryczna we Wroc³awiu, KRS 0000002736, NIP 895-10-07-276, Kapita³ zak³adowy zarejestrowany i op³acony 31 500 000 z³</span>	
		...
		*/
		$header = '';
		//$header = '<div  style="margin-right:0px; width: 920px; text-align: left; margin-top: 0px;border: #FFF solid 1px; "><img src="graphics/europa_papie_firmowy_head2.jpg" /></div>';
		//$STOPKA_F = '<div  style="margin-right:0px; width: 920px; text-align: left; margin-top: 0px;border: #FFF solid 1px; "><img src="graphics/europa_papie_firmowy_footer.png" style="margin-right:20px"/></div>';
		$STOPKA_F='';
	//	$watermark = '<img src="graphics/europa_papie_firmowy.jpg">';
		$watermark= dirname(__FILE__).'/../fax/templates/paper/europa_background3.pdf';

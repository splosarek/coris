...
include_once('lib/lib_europa.php');
  ...
  if (count($pozycje) > 0 ) {
      ...
      $query = "SELECT * FROM coris_europa_claims_details  WHERE ID IN (".implode(',', $pozycje).") ";
      //echo $query;
      ...
      $query = "SELECT * FROM  coris_europa_claims WHERE ID='".$lista_roszczen[0][ID_claims]."'";
      $mysql_result = mysql_query($query);
...
//      $page_template->set('<!--TEXT_1-->' , getTxt1($row['payment_amount']) );
     ...
     // 	$body .=  $page_template->realize();
      ...
      $body .=  $page_template->realize();      	        
  		...
  		$tmp = file("../fax/templates/europa_decyzja_header.html");
  		$header = implode('', $tmp);
...
function generateTxt($typ,$case_id,$nr){
		...
		$row_case_announce = EuropaCase::getCaseeuropaInfo($case_id);
		$row_case= CaseInfo::getFullCaseInfo($case_id);

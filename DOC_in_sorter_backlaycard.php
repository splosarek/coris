...
function load_documents(){
	load_ayax('document_list','ayax/sorter_doc_list_barclaycard.php');
}


function load_document(id){
	load_ayax('document_preview','ayax/doc_view_barclaycard.php?id='+id );
}

function load_case(){
	load_ayax('case_list','ayax/sorter_case_list_barclaycard.php');
}

function register_new_case(doc_id,att_id){

    window.open('AS_cases_add_med.php?tryb=barcleycard&doc_id='+doc_id+"&att_id="+att_id, 'newcase_<?php echo time() ?>', 'toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=1100,height=750,left=0,top=0');
}

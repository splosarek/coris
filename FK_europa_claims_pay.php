...
function naglowek(){
  ...
  <tr valign="baseline" bgcolor="#CCCCCC">
    <td align="center" nowrap style="border: #000000 1px solid;"><strong>Europa roszczenia p³atno¶ci</strong>
    </td></tr>
  <tr valign="baseline" bgcolor="#CCCCCC">
...
<form method="post">	
	...
	if ($action_list==1){
		...
		if (is_array($f_payment_date)){
			foreach ($f_payment_date As $key => $val ){
				if ($val != ''){
					$query = "UPDATE coris_europa_payment SET payment=1, payment_date='$val' WHERE ID='$key' LIMIT 1";
					//echo "<br>$query";
					mysql_query($query);
		...
		if (is_array($f_refund_date)){
			foreach ($f_refund_date As $key => $val ){
				if ($val != ''){
					$query = "UPDATE coris_europa_payment SET refund =1, refund_date ='$val' WHERE ID='$key' LIMIT 1";
					mysql_query($query);
					//echo "<br>R $key $val";
				}
			}
    ...
    coris_europa_payment cep,  coris_europa_decisions_details cedd, coris_europa_claims_details cecd,
    coris_europa_decisions ced
    WHERE 
		...
		lista = document.getElementsByName(\'pozcyje[]\');
 		...
 		if (lista_pozycji == \'\'){
 			alert(\'Zaznacz pozycje do drukowania\');
				...
				MM_openBrWindow(\'FK_europa_claims_pay_print.php?lista=\'+lista_pozycji,\'\',\'scrollbars=yes,resizable=yes,width=1500,height=800,left=20,top=20\');
		}
    ...
    <th width="80">Nr umowy ubezp.</th>
  	...
  	$i=0;
  	 while ($row = mysql_fetch_array($mysql_result)){  	  				  	  	
		...
		$row_case_ann = EuropaCase::getCaseeuropaInfo( $row['ID_case'] );
      
  		$result .= '<tr bgcolor="'.( ($i++ % 2) ? "#FFFFFF" : "#EEEEEE" ) .'">
  			<td nowrap><input type="checkbox" name="pozcyje[]" value="'.$row['ID'].'" style="width: 20px" ></td>
  			<td nowrap><input type="button" value="&gt;" style="width: 20px" onClick="open_case2(\''.$row['ID_case'].'&mod=europa_claims\',\'\');"></td>
  		';

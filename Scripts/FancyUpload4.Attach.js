...
if (!window.FancyUpload4) var FancyUpload4 = {};

FancyUpload4.Attach = new Class({
  ...
  initialize: function(list, selects, options) {
    ...
    options.fileClass = options.fileClass || FancyUpload4.Attach.File;
  ...
  getID2: function(){
	  return this.options.fileClass.getID();
  ...
  ,
  getId: function() {	 
	  return this.options.fileClass;
  }
});

FancyUpload4.Attach.File = new Class({
 ...
 //   this.ui.title = new Element('a', {'class': 'file-title', text: this.name.substr(0,25), href:  this.url , title: this.name});
    ...
    var progress = new Element('img', {'class': 'file-progress', src: 'img/assets/progress-bar/bar.gif'}).inject(this.ui.size, 'after');
...
(function() {
  ...
  if (MooTools.lang) {
   			// MooTools.lang.set('pl-PL', 'FancyUpload', phrases);

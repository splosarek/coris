...
$expenses_id = getValue('eid');
$claim_id = getValue('cid');

$case_id = getValue('case_id');
$result = array();
		
if ($case_id>0){	
		$rezerwy_all = CaseInfo::getReserve($case_id);
		
		if ($expenses_id>0){
			$rezerwa_exp = CaseInfo::getReserve($case_id,$expenses_id);
		}else if ($claim_id>0){
			$rezerwa_exp = CaseInfo::getReserve($case_id,0,$claim_id);
		}else{
			$rezerwa_exp = array('rezerwa' => 0.0,'currency_id'=>'PLN' );
		}	
		
		$corisCase = new CorisCase($case_id);
		$su = $corisCase->getPolicyamount();
		$su_waluta = $corisCase->getPolicycurrency_id();
		
		$tmp_rez_glob = CaseInfo::getGLobalReserve($case_id);
		$rezerwy_globalna = $tmp_rez_glob['rezerwa'];
		
		 if ($claim_id>0 || $claim_id==-1){
			...
			if ($currency=='PLN'){
				...
				$mysql_result = CaseCurrency::getKurs($currency_date,$table_invoice,$currency,$table_source,0);
		...
		$result = array('suma'=> $su,'suma_waluta' => $su_waluta,'rezerwa_globalna' => $rezerwy_globalna, 'rezerwa_all' => $rezerwy_all['rezerwa'], 'rezerwa' => $rezerwa_exp['rezerwa'],'rezerwa_waluta' => $rezerwy_all['currency_id'],'currency_table_id' => $table_id, 'currency_rate' => $rate);						
}else{
		$result = array('suma'=> 0,'suma_waluta' => 'PLN', 'rezerwa_globalna' => 0,'rezerwa' => 0, 'rezerwa' => 0,'rezerwa_waluta' => 'PLN','currency_table_id' => 0, 'currency_rate' => 1 );
}

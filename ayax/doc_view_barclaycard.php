  	...
  	$result .=  '
    	...
    	<table width="100%" border="0" cellpadding="0" cellspacing="2">
  		...
  		foreach ($att As $attachment){
    			...
    			if ( $file_name == 'register.bin' && $register_new_case==false){
                    $register_new_case=true;
                    $attach_id=$attachment->getObjectID();
                }
...
<div id="case_add_form" name="case_add_form" style="position:relative; display:block;height:auto;margin-left:5px;">
   ...
   if (count($att) > 0 ){
    ...
    if ($register_new_case){
    $result .= '<tr  bgcolor="#EEEEEE">
    	<td align="center" nowrap style="border: #000000 1px solid;" colspan="3">';
    $result .= '<input type="button" value="Za³ó¿ now± sprawê" style="width: 300px;text-align: center;background-color: red;color: white" onclick="register_new_case('.$document->getObjectID().','.$attach_id.')">';
    ...
    if ($tryb=='print'){
    	...
    	$result .= '<div style="display: block; width: 590px;height: 400px;overflow:auto;background-color:#FFFFFF">';
    			...
    			if ($document->get_body_html()){
    					$result .= strtr($document->getBody(),array('<script type="text/javascript">'=>'','<script>' => '','</script>'=> ''));

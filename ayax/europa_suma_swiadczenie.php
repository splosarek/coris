...
//echo $query;
		...
		if ($row['ID_podlimit'] > 0 ){
			...
			if ($waluta2 != 'PLN')
				$limit2 = $limit2*$europa_case->getKursWaluty($waluta2);		
		}
		
		$limit = $row['kwota'];
		$waluta = $row['currency_id'];
		if ($waluta != 'PLN')
			$limit = $limit*$europa_case->getKursWaluty($waluta);
		...
		$rezerwa_globalna = 	$europa_case->getRezerwaGlobalna();
...
function sprawdzAktualneUzycie($case_id,$zid){
		global $europa_case;
	$query = "SELECT * FROM  coris_europa_rezerwy  WHERE case_id = '$case_id' ";
	$mysql_result = mysql_query($query);
	$suma_globalna=0.0;
	$suma = 0.0;
	while ($row=mysql_fetch_array($mysql_result)) {
		...
		$waluta = $row['currency_id'];
		if ($waluta!= 'PLN')
			$rezerwa = $rezerwa*$europa_case->getKursWaluty($waluta);

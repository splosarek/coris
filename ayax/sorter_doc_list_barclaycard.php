...
$param = array('body' => $search_txt,'data_od' => $data_od, 'data_do' => $data_do , 'status' => $status,'destination_id' => 11,'page'=> $page,'count_per_page' => $count_per_page);
...
$result .= '<table  WIDTH="374" cellpadding="1" cellspacing="0" style="border: #cccccc 1px solid;">
  ...
  <tr valign="baseline" bgcolor="#FFFFFF">
	...
	<tr align="center" bgcolor="#CCCCCC">
		...
		<th align="center" width="220">Rodzaj</th>
  ...
  foreach ($lista_doc As $document ){
	...
	$result .= '<tr>
       ...
       }if ($document->get_destination() == 12 ) {
           $target_style = 'style="color:green;"';
	...
	$result  .= '</td>
	  		<td align="center" nowrap><span class="style4" '.$target_style.'>'.$st1.$document->getCreateDate().$st2.'</span></td>';
					$result .= '<td align="center" title="">'.$st1;
						$result .= htmlspecialchars(substr($document->getName(),0,40),ENT_QUOTES ,'ISO-8859-1');
					$result .= $st2.'</td>';
      ...
      //<td align="center" nowrap>'.$st1.$ilosc.$st2.'</td>
	  		$result .= '
	  	</tr>';
  }
  	$result .= '</table>
...
<script>
 function refresh_form_doc_list(){
	save_form(\'doc_list_result\',\'form_doc_list\',\'ayax/sorter_doc_list_barclaycard.php\');
}
</script>
';

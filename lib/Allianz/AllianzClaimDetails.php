...
class AllianzClaimDetails{
	...
	var $status2_lista = array('','Wys³ane do akceptacji','Do poprawy','Zaakceptowane');
	...
	private function load(){
			$query = "SELECT * FROM coris_allianz_claims_details  WHERE ID = '$this->ID'";
	...
	function zmienStatus2(){
			//if ($this->old_values['status2'] != $this->getStatus2() ){									
						...
						$query = "UPDATE  coris_allianz_claims_details SET 															
						$zm_status																		
	...
	public function store(){						
			if ($this->ID > 0 ){				
				if ($this->delete){
					$query = "DELETE FROM coris_allianz_claims_details WHERE ID = '$this->ID' LIMIT 1";
					$mysql_result = mysql_query($query);
					if (!$mysql_result){echo "<br>QE: $query <br>".mysql_error(); }		
				}else{
					$zm_status='';
				...
				$query = "INSERT INTO coris_allianz_claims_details SET 
...
*/
	...
	public function getStatusName($state=0){
		...
		$query = "SELECT nazwa  FROM coris_allianz_claims_details_status WHERE ID='$state' "; 
		$mysql_result = mysql_query($query);			
		$row = mysql_fetch_array($mysql_result);
		return $row['nazwa'];				
	}
	...
	public function updateLog(){
			$query = "INSERT INTO coris_allianz_claims_details_status_log 
	...
	public function createDecision($case_id){
		if ($case_id>0){
			$refundacja = 0.0;
			$franszyza = 0.0;
			$odliczenia = 0.0;
			
			if (  $this->getRefundacja()  )
					$refundacja = $this->getRefundacja_kwota();

			if (  $this->getFranszyza()  )
					$franszyza = $this->getFranszyza_kwota();

			if (  $this->getOdliczenie()  )
					$odliczenia = $this->getOdliczenie_kwota();

					
			$query = "INSERT INTO coris_allianz_decisions 
			SET ID_case='$case_id',
			ID_claims_details = '".$this->getID()."',
			ID_user = '".$this->getStatus_userID()."',
			ID_user2 = '".$this->getStatus2_userID()."',
			`type`='".$this->getStatus()."',
			`date`=now(),
			`status`=0,text1='',text2='',amount='".$this->getWyplata_zaakceptowana()."',franszyza='$franszyza',odliczenia='$odliczenia',amount2='".$refundacja."',payment_amount='".($this->getWyplata_zaakceptowana()+$refundacja)."';
			;
			";
			$mysql_result = mysql_query($query);	
			if (!$mysql_result){echo "<br>QE: $query <br>".mysql_error(); }		
	...
	public function createPayment($case_id){
		if ($case_id>0){
			/*if ($this->getStatus()==3){//poztywna
				$kwota = $this->getWyplata_zaakceptowana();
				$kwota2 = 40;
				$wyplata = $kwota+$kwota2 ;
			}
			
			if ($this->getStatus()==4){//odmowa
				$kwota = 0;
				$kwota2 = 40;
				$wyplata = $kwota+$kwota2;
			}*/
				...
				$kwota = $this->getWyplata_zaakceptowana();
				if ( $this->getRefundacja() == 1 ){
					$kwota2 = $this->getRefundacja_kwota();					
				}else{
					$kwota2=0.0;					
				}			
				$wyplata = $kwota+$kwota2 ;
				...
				if ($wyplata > 0.0){
					$query = "INSERT INTO coris_allianz_payment 
					SET ID_case='$case_id',
					ID_claims_details = '".$this->getID()."',			
					date=now(),
					status=0,amount='".$kwota."',amount2='".$kwota2."',payment_amount='".($wyplata)."'";
					$mysql_result = mysql_query($query);	
					if (!$mysql_result){echo "<br>QE: $query <br>".mysql_error(); }
				}					
		}
	...
	public function DeletePayment(){		
		$query = "DELETE FROM coris_allianz_payment 
	...
	public function DeleteDecision(){		
		$query = "DELETE FROM coris_allianz_decisions 
			WHERE  ID_claims_details = '".$this->getID()."'";
			$mysql_result = mysql_query($query);	
			if (!$mysql_result){echo "<br>QE: $query <br>".mysql_error(); }				
	}

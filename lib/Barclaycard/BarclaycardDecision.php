...
class BarclaycardDecision{
	...
	static public function createDecision($case_id,$EuropaClaimDetailsList){
		if ($case_id>0){
			...
			$query = "INSERT INTO coris_barclaycard_decisions 
			SET ID_case='$case_id',			
			...
			foreach ($EuropaClaimDetailsList As $cls){
				...
				$query = "INSERT INTO coris_barclaycard_decisions_details 
			...
			$query = "UPDATE coris_barclaycard_decisions SET payment_amount='$suma',type='$type' WHERE ID='$ced' LIMIT 1";
			$mysql_result = mysql_query($query);	
			if (!$mysql_result){echo "<br>QE: $query <br>".mysql_error(); }		
		}
	}
	...
	private function load(){
			$query = "SELECT * FROM coris_barclaycard_decisions  WHERE ID = '$this->ID'";
			$mysql_result = mysql_query($query);
			if (mysql_num_rows($mysql_result) >0){
				$row = mysql_fetch_array($mysql_result);
					
				$this->setID_claims_details($row['ID_claims_details']);
				$this->claims_details = new BarclaycardClaimDetails($this->getID_claims_details());
				...
				$query = "SELECT * FROM coris_barclaycard_decisions_details  WHERE ID_decisions = '$this->ID' ORDER BY ID";
				$mysql_result = mysql_query($query);
				if (mysql_num_rows($mysql_result) >0){
...
function generateTxt($nr){
		...
		$row_case_announce = BarclaycardCase::getCaseAnnounce($this->ID_case);
...
function getTxt1($row_case,$row_case_announce){
	...
	if ( $this->getType()==3 ){  // poz
		...
		$result .= "\n\n".'wir haben die Leistungsprüfung abgeschlossen und die geltend gemachten Ansprüche anerkannt.  Folgender Betrag  ist fällig geworden: <!--KWOTA-->';
	//	$result = '';
	}	
...
//		if ( $franszyza_rodzaj ==2 && $this->getClaims_details()->getWyplata_zaakceptowana() < $this->getClaims_details()->getKwota_zaakceptowana()){
	...
	static function updateDecision($decisions_id, $tekst1, $tekst2, $tekst3, $tekst4, $tekst5, $data_decyzji){
		...
		$query = "SELECT * FROM coris_barclaycard_decisions_details WHERE ID_decisions='$decisions_id'";
		$mysql_result = mysql_query($query);	
		$suma = '0.0';
		while ($row = mysql_fetch_array($mysql_result)){
			...
			if ($row['currency_id'] != 'EUR' && $row['amount'] > 0.0){
					...
					$qu = "UPDATE coris_barclaycard_decisions_details 						
		...
		$query = "UPDATE coris_barclaycard_decisions SET
			status=1,

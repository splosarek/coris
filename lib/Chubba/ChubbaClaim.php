...
class ChubbaClaim{
	...
	private function load(){
			$query = "SELECT * FROM ".ChubbaCase::$TABLE_CLAIMS."  WHERE ID = '$this->ID'";
			...
			if (mysql_num_rows($mysql_result) >0){
				...
				$query2 = "SELECT ID FROM ".ChubbaCase::$TABLE_CLAIMS_DETAILS."  WHERE ID_claims = '".$this->ID."'";
				$mysql_result = mysql_query($query2);			
				while ($row2 = mysql_fetch_array($mysql_result)){
						$this->addClaimDetails(new ChubbaClaimDetails($row2['ID'],$this->ID) );
				}				
			}
	}
	...
	public function store(){
			...
			if ($this->new_claims){  //INSERT			
					$query = "INSERT INTO ".ChubbaCase::$TABLE_CLAIMS." SET 
					ID_case='$this->caseID',
					...
					$query = "UPDATE  ".ChubbaCase::$TABLE_CLAIMS." SET
						announce_date='".$this->announce_date."',
						note='".mysql_escape_string(stripslashes($this->note))."',
						ID_user='".$_SESSION['user_id']."',
						date=now()

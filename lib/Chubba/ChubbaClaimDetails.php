...
class ChubbaClaimDetails{
	...
	private function load(){
			$query = "SELECT * FROM ".ChubbaCase::$TABLE_CLAIMS_DETAILS."  WHERE ID = '$this->ID'";
	...
	function zmienStatus2(){
			//if ($this->old_values['status2'] != $this->getStatus2() ){									
						...
						$query = "UPDATE  ".ChubbaCase::$TABLE_CLAIMS_DETAILS." SET 															
						$zm_status																		
	...
	public function store(){						
			if ($this->ID > 0 ){				
				if ($this->delete){
					$query = "DELETE FROM ".ChubbaCase::$TABLE_CLAIMS_DETAILS." WHERE ID = '$this->ID' LIMIT 1";
					$mysql_result = mysql_query($query);
					if (!$mysql_result){echo "<br>QE: $query <br>".mysql_error(); }		
				}else{
					$zm_status='';
				...
				$query = "INSERT INTO ".ChubbaCase::$TABLE_CLAIMS_DETAILS." SET 
...
*/
	...
	public function getStatusName($state=0){
		...
		$query = "SELECT nazwa  FROM ".ChubbaCase::$TABLE_CLAIMS_DETAILS_STATUS." WHERE ID='$state' ";
		$mysql_result = mysql_query($query);			
		$row = mysql_fetch_array($mysql_result);
		return $row['nazwa'];				
	}
	...
	public function updateLog(){
			$query = "INSERT INTO ".ChubbaCase::$TABLE_CLAIMS_DETAILS_STATUS_LOG." 
	...
	public function createPayment($case_id,$kwota){
		...
		if ($case_id>0){
				if ($kwota > 0.0){
					$query = "INSERT INTO ".ChubbaCase::$TABLE_PAYMENT." 
					SET ID_case='$case_id',
	...
	public function DeletePayment(){		
		$query = "DELETE FROM ".ChubbaCase::$TABLE_PAYMENT."  
	...
	public function DeleteDecision(){
		...
		$query = "SELECT * FROM ".ChubbaCase::$TABLE_CLAIMS_DECISIONS_DETAILS."  WHERE ID_claims_details = '".$this->getID()."'";
		$mr = mysql_query($query);
		while($row=mysql_fetch_array($mr)){
			
			$query = "DELETE FROM ".ChubbaCase::$TABLE_CLAIMS_DECISIONS." 
			WHERE  ID = '".$row['ID_decisions']."'";
			$mysql_result = mysql_query($query);	
		}
		
		$query = "DELETE FROM ".ChubbaCase::$TABLE_CLAIMS_DECISIONS_DETAILS." 

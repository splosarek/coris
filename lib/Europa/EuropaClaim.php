...
class EuropaClaim{
	...
	private function load(){
			$query = "SELECT * FROM coris_europa_claims  WHERE ID = '$this->ID'";			
			...
			if (mysql_num_rows($mysql_result) >0){
				...
				$query2 = "SELECT ID FROM coris_europa_claims_details  WHERE ID_claims = '".$this->ID."'";
				$mysql_result = mysql_query($query2);			
				while ($row2 = mysql_fetch_array($mysql_result)){
						$this->addClaimDetails(new EuropaClaimDetails($row2['ID'],$this->ID) );
				}				
			}
	}
	...
	public function store(){
			...
			if ($this->new_claims){  //INSERT			
					$query = "INSERT INTO coris_europa_claims SET 
					ID_case='$this->caseID',
					...
					$query = "UPDATE  coris_europa_claims SET
						announce_date='".$this->announce_date."',
						note='".mysql_escape_string(stripslashes($this->note))."',
						ID_user='".$_SESSION['user_id']."',
						date=now()

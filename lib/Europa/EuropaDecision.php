...
class EuropaDecision{
	...
	static public function createDecision($case_id,$EuropaClaimDetailsList){
		if ($case_id>0){
			...
			$query = "INSERT INTO coris_europa_decisions 
			SET ID_case='$case_id',			
			...
			foreach ($EuropaClaimDetailsList As $cls){
				...
				$query = "INSERT INTO coris_europa_decisions_details 
			...
			$query = "UPDATE coris_europa_decisions SET payment_amount='$suma',type='$type' WHERE ID='$ced' LIMIT 1";
			$mysql_result = mysql_query($query);	
			if (!$mysql_result){echo "<br>QE: $query <br>".mysql_error(); }		
		}
	}
	...
	private function load(){
			$query = "SELECT * FROM coris_europa_decisions  WHERE ID = '$this->ID'";
			$mysql_result = mysql_query($query);
			if (mysql_num_rows($mysql_result) >0){
				$row = mysql_fetch_array($mysql_result);
					
				$this->setID_claims_details($row['ID_claims_details']);
				$this->claims_details = new EuropaClaimDetails($this->getID_claims_details());
				...
				$query = "SELECT * FROM coris_europa_decisions_details  WHERE ID_decisions = '$this->ID' ORDER BY ID";
				$mysql_result = mysql_query($query);
				if (mysql_num_rows($mysql_result) >0){
...
function generateTxt($nr){
		...
		$row_case_announce = EuropaCase::getCaseeuropaInfo($this->ID_case);
...
function getTxt1($row_case,$row_case_announce){
	...
	if ( $this->getType()==3 ){  // poz
		$result = 'W zwi±zku ze zg³oszonymi roszczeniami APRIL Polska Sp. z o.o. dzia³aj±c w imieniu TU EUROPA S.A. refunduje poniesione koszty leczenia w wysoko¶ci <!--KWOTA-->';
	//	$result = '';
	}	
	
	if ( $this->getType()==4 ) // odm
		$result = 'W zwi±zku ze zg³oszonymi roszczeniami APRIL Polska Sp. z o.o. dzia³aj±c w imieniu TU EUROPA S.A. oddala zg³oszone roszczenia dotycz±ce ...... .';
...
function getTxt2($row_case,$row_case_announce){
	if ( $this->getType()==3 ){  // poz
		...
		$result .= "\n\n".'Podstaw± do wyliczenia nale¿no¶ci jest ¶redni kurs walut wg NBP obowi±zuj±cy w dniu wydania decyzji o wyp³acie ¶wiadczenia ubezpieczeniowego, zgodnie z § 10 ust. 8 Ogólnych Warunków Ubezpieczenia Twoja Karta Podró¿e TU Europa S.A.';
...
//		if ( $franszyza_rodzaj ==2 && $this->getClaims_details()->getWyplata_zaakceptowana() < $this->getClaims_details()->getKwota_zaakceptowana()){
	...
	static function updateDecision($decisions_id, $tekst1, $tekst2, $tekst3, $tekst4, $tekst5, $data_decyzji){
		...
		$query = "SELECT * FROM coris_europa_decisions_details WHERE ID_decisions='$decisions_id'";
		$mysql_result = mysql_query($query);	
		$suma = '0.0';
		while ($row = mysql_fetch_array($mysql_result)){
			...
			if ($row['currency_id'] != 'PLN' && $row['amount'] > 0.0){				
					...
					$qu = "UPDATE coris_europa_decisions_details 						
		...
		$query = "UPDATE coris_europa_decisions SET
			status=1,

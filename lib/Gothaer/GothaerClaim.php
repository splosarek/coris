...
class GothaerClaim{
	...
	private function load(){
			$query = "SELECT * FROM ".GothaerCase::$TABLE_CLAIMS."  WHERE ID = '$this->ID'";
			...
			if (mysql_num_rows($mysql_result) >0){
				...
				$query2 = "SELECT ID FROM ".GothaerCase::$TABLE_CLAIMS_DETAILS."  WHERE ID_claims = '".$this->ID."'";
				$mysql_result = mysql_query($query2);			
				while ($row2 = mysql_fetch_array($mysql_result)){
						$this->addClaimDetails(new GothaerClaimDetails($row2['ID'],$this->ID) );
				}				
			}
	}
	...
	public function store(){
			...
			if ($this->new_claims){  //INSERT			
					$query = "INSERT INTO ".GothaerCase::$TABLE_CLAIMS." SET 
					ID_case='$this->caseID',
					...
					$query = "UPDATE  ".GothaerCase::$TABLE_CLAIMS." SET
						announce_date='".$this->announce_date."',
						note='".mysql_escape_string(stripslashes($this->note))."',
						ID_user='".$_SESSION['user_id']."',
						date=now()
					...
					*/
					...
					if (count($update_roznice) > 0 ){
											VIGCase::rejestrujZmiany($this->caseID,'CLAIMS','UPDATE',$update_roznice);
					}
					*/
			}
	}

...
class GothaerDecision{
	...
	static public function createDecision($case_id,$VIGClaimDetailsList){
		if ($case_id>0){
			...
			$query = "INSERT INTO ".GothaerCase::$TABLE_CLAIMS_DECISIONS." 
			SET ID_case='$case_id',			
			...
			foreach ($VIGClaimDetailsList As $cls){
				...
				$query = "INSERT INTO ".GothaerCase::$TABLE_CLAIMS_DECISIONS_DETAILS." 
			...
			$query = "UPDATE ".GothaerCase::$TABLE_CLAIMS_DECISIONS." SET payment_amount='$suma',type='$type' WHERE ID='$ced' LIMIT 1";
			$mysql_result = mysql_query($query);	
			if (!$mysql_result){echo "<br>QE: $query <br>".mysql_error(); }		
		}
	}
	...
	private function load(){
			$query = "SELECT * FROM ".GothaerCase::$TABLE_CLAIMS_DECISIONS."  WHERE ID = '$this->ID'";
			$mysql_result = mysql_query($query);
			if (mysql_num_rows($mysql_result) >0){
				$row = mysql_fetch_array($mysql_result);
					
				$this->setID_claims_details($row['ID_claims_details']);
				$this->claims_details = new GothaerClaimDetails($this->getID_claims_details());
				...
				$query = "SELECT * FROM ".GothaerCase::$TABLE_CLAIMS_DECISIONS_DETAILS."  WHERE ID_decisions = '$this->ID' ORDER BY ID";
				$mysql_result = mysql_query($query);
				if (mysql_num_rows($mysql_result) >0){
...
function generateTxt($nr){
		...
		$row_case_announce = GothaerCase::getCaseInfo($this->ID_case);
...
function getTxt1($row_case,$row_case_announce){
	...
	if ( $this->getType()==3 ){  // poz
        ...
        $result .= "\n\n";
        $result .= 'W odpowiedzi na korespondencjê otrzyman± w dniu <!--DATA_ZGLOSZENIA--> April Polska Sp. o.o. dzia³aj±c w imieniu i na rzecz Gothaer TU SA  na podstawie art. 29 w zwi±zku z art. 28 ustawy o dzia³alno¶ci ubezpieczeniowej z dnia 11 wrze¶nia 2015 r. uprzejmie informuje, ¿e po rozpatrzeniu wniesionych roszczeñ, dotycz±cych szkody powsta³ej w dniu <!--DATA_ZDARZENIA--> wynikaj±cej z polisy ubezpieczeniowej <!--POLISA--> wyp³aci kwotê w wysoko¶ci: <!--KWOTA-->.';
	//	$result = '';
	}	
...
function getTxt2($row_case,$row_case_announce){
	if ( $this->getType()==3 ){  // poz
		$result = 'Uprzejmie informujemy, i¿ podstaw± do wyliczenia nale¿no¶ci jest ¶redni kurs NBP obowi±zuj±cy w dniu ustalenia ¶wiadczenia lub odszkodowania, § 14 ust. 4 Warunków Ubezpieczenia kosztów leczenia za granic± i poza krajem sta³ego pobytu dla posiadaczy Kart Planeta M³odych i Kart Planeta M³odych Plus.';
		$result  .= "\n\n";
	...
	if ( $this->getType()==4 ) {// odm
        $result = 'Wyja¶niamy, i¿ zgodnie z  § ... Warunków Ubezpieczenia Kosztów leczenia za granic± i poza krajem sta³ego pobytu dla posiadaczy Kart Planeta M³odych i Kart Planeta M³odych Plus: 

Jak wynika z dokumentacji zgromadzonej w przedmiotowej sprawie ...

Maj±c na uwadze powy¿sze zmuszeni jeste¶my oddaliæ roszczenia wnoszone w zakresie ...
';
    }
	return $result;
}
...
//		if ( $franszyza_rodzaj ==2 && $this->getClaims_details()->getWyplata_zaakceptowana() < $this->getClaims_details()->getKwota_zaakceptowana()){
	...
	static function updateDecision($decisions_id, $tekst1, $tekst2, $tekst3, $tekst4, $tekst5,$tekst6, $data_decyzji){
		...
		$query = "SELECT * FROM ".GothaerCase::$TABLE_CLAIMS_DECISIONS_DETAILS." WHERE ID_decisions='$decisions_id'";
		$mysql_result = mysql_query($query);	
		$suma = '0.0';
		while ($row = mysql_fetch_array($mysql_result)){
			...
			if ($row['currency_id'] != 'PLN' && $row['amount'] > 0.0){				
					...
					$qu = "UPDATE ".GothaerCase::$TABLE_CLAIMS_DECISIONS_DETAILS." 						
		...
		$query = "UPDATE ".GothaerCase::$TABLE_CLAIMS_DECISIONS." SET
			status=1,

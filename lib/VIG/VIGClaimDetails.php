...
class VIGClaimDetails{
	...
	private function load(){
			$query = "SELECT * FROM coris_vig_claims_details  WHERE ID = '$this->ID'";
	...
	function zmienStatus2(){
			//if ($this->old_values['status2'] != $this->getStatus2() ){									
						...
						$query = "UPDATE  coris_vig_claims_details SET 															
						$zm_status																		
	...
	public function store(){						
			if ($this->ID > 0 ){				
				if ($this->delete){
					$query = "DELETE FROM coris_vig_claims_details WHERE ID = '$this->ID' LIMIT 1";
					$mysql_result = mysql_query($query);
					if (!$mysql_result){echo "<br>QE: $query <br>".mysql_error(); }		
				}else{
					$zm_status='';
				...
				$query = "INSERT INTO coris_vig_claims_details SET 
...
*/
	...
	public function getStatusName($state=0){
		...
		$query = "SELECT nazwa  FROM coris_vig_claims_details_status WHERE ID='$state' "; 
		$mysql_result = mysql_query($query);			
		$row = mysql_fetch_array($mysql_result);
		return $row['nazwa'];				
	}
	...
	public function updateLog(){
			$query = "INSERT INTO coris_vig_claims_details_status_log 
	...
	public function createPayment($case_id,$kwota){
		...
		if ($case_id>0){
				if ($kwota > 0.0){
					$query = "INSERT INTO coris_vig_payment 
					SET ID_case='$case_id',
	...
	public function DeletePayment(){		
		$query = "DELETE FROM coris_vig_payment 
	...
	public function DeleteDecision(){
		...
		$query = "SELECT * FROM coris_vig_decisions_details WHERE ID_claims_details = '".$this->getID()."'";
		$mr = mysql_query($query);
		while($row=mysql_fetch_array($mr)){
			
			$query = "DELETE FROM coris_vig_decisions 
			WHERE  ID = '".$row['ID_decisions']."'";
			$mysql_result = mysql_query($query);	
		}
		
		$query = "DELETE FROM coris_vig_decisions_details 

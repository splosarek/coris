...
class VIGDecision{
	...
	static public function createDecision($case_id,$VIGClaimDetailsList){
		if ($case_id>0){
            ...
            if ($coris_case->getClient_id() == 7 ){
                $data_kursu = $coris_case->getEventdate();
            }


			$query = "INSERT INTO coris_vig_decisions 
			SET ID_case='$case_id',			
			...
			foreach ($VIGClaimDetailsList As $cls){
				if ($cls->getWaluta() != 'PLN'){
					$kurs = Finance::getKursTD($data_kursu,1,$cls->getWaluta());
				...
				$query = "INSERT INTO coris_vig_decisions_details 
			...
			$query = "UPDATE coris_vig_decisions SET payment_amount='$suma',type='$type' WHERE ID='$ced' LIMIT 1";
			$mysql_result = mysql_query($query);	
			if (!$mysql_result){echo "<br>QE: $query <br>".mysql_error(); }		
		}
	}
	...
	private function load(){
			$query = "SELECT * FROM coris_vig_decisions  WHERE ID = '$this->ID'";
			$mysql_result = mysql_query($query);
			if (mysql_num_rows($mysql_result) >0){
				$row = mysql_fetch_array($mysql_result);
					
				$this->setID_claims_details($row['ID_claims_details']);
				$this->claims_details = new VIGClaimDetails($this->getID_claims_details());
				...
				$query = "SELECT * FROM coris_vig_decisions_details  WHERE ID_decisions = '$this->ID' ORDER BY ID";
				$mysql_result = mysql_query($query);
				if (mysql_num_rows($mysql_result) >0){
...
function generateTxt($nr){
		...
		$row_case_announce = VIGCase::getCaseInfo($this->ID_case);
...
//		if ( $franszyza_rodzaj ==2 && $this->getClaims_details()->getWyplata_zaakceptowana() < $this->getClaims_details()->getKwota_zaakceptowana()){
	...
	static function updateDecision($case_id,$decisions_id, $tekst1, $tekst2, $tekst3, $tekst4, $tekst5, $data_decyzji){
		
		$query = "SELECT * FROM coris_vig_decisions_details WHERE ID_decisions='$decisions_id'";
		$mysql_result = mysql_query($query);	
		$suma = '0.0';

        $data_kursu = $data_decyzji;
		...
		while ($row = mysql_fetch_array($mysql_result)){
			...
			if ($row['currency_id'] != 'PLN' && $row['amount'] > 0.0){				
					$kurs = Finance::getKursTD($data_kursu,1,$row['currency_id']);
					$tabled_id = $kurs['table_id'] ;
					$rate = $kurs['rate'];
					$multiplier = $kurs['multiplier'];
					
					$platnosc = Finance::ev_round( ( $row['amount'] * $rate) / $multiplier ,2);										
					$qu = "UPDATE coris_vig_decisions_details 						
		...
		$query = "UPDATE coris_vig_decisions SET
			status=1,

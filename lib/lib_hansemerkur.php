<?php
include(dirname(__FILE__).'/Hansemerkur/HansemerkurClaim.php');
include(dirname(__FILE__).'/Hansemerkur/HansemerkurClaimDetails.php');
include(dirname(__FILE__).'/Hansemerkur/HansemerkurDecision.php');

Class HansemerkurCase{

    static $TABLE_ANNONUNCE = 'coris_hansemerkur_announce';
    static $TABLE_program = 'coris_hansemerkur_wariant_umowy';
	...
	function  getBiurPodrozyNazwa(){
		$query = "SELECT nazwa FROM coris_hansemerkur_biura_podrozy  WHERE ID = '".$this->_biuro_podrozy."'";
	...
	function  getOpcjeUmowy(){
		$query = "SELECT ID_opcja FROM coris_hansemerkur_announce_opcje WHERE case_id = '".$this->case_id."'";
		$mysql_result = mysql_query($query);
		$res = array();
	...
	function setStatus($new_status){					
		if ( $this->case_id > 0 && $new_status>0 ){						
			...
			$mysql_result = mysql_query($query);
			if ($mysql_result){
					$qu = "INSERT INTO coris_hansemerkur_status_historia SET case_id='".$this->case_id."', ID_status='$new_status',ID_user='".$_SESSION['user_id']."',date=now();";										
					$mr = mysql_query($qu);										
					if (!$mr){
	...
	static  function  wysw_status($name,$def,$tryb=0,$option=""){
		$result='';		
		if ($tryb){
				$query = "SELECT * FROM coris_hansemerkur_status_szkody    WHERE  ID='$def' ORDER BY  kolejnosc";
	//echo $query;
				$mysql_result = mysql_query($query);
				$row = mysql_fetch_array($mysql_result);			
			...
			$result = '<select name="'.$name.'" id="'.$name.'" style="font-size: 8pt;width: 200px;"  '.$option.'>';
				...
				$query = "SELECT * FROM coris_hansemerkur_status_szkody  ORDER BY kolejnosc";
	...
	if ($tryb){
			$query = "SELECT * FROM coris_hansemerkur_typ_umowy   WHERE ID_client='$client_id' AND ID='$def' ORDER BY nazwa";		
		...
		$result = '<select name="'.$name.'" id="'.$name.'" style="font-size: 8pt;width: 200px;"  '.$option.'>';
		//<option value=""></option>
			$query = "SELECT * FROM coris_hansemerkur_typ_umowy WHERE ID_client='$client_id'   ORDER BY nazwa";						
	...
	if ($tryb){
			$query = "SELECT * FROM coris_hansemerkur_zakres_ubezpieczenia    WHERE ID='$def' ORDER BY kolejnosc,nazwa";		
		...
		if ($def>0){
			...
			$qt = "SELECT * FROM coris_hansemerkur_rezerwy  WHERE case_id ='".$case_id."'";
			$mt = mysql_query($qt);
		...
		//<option value=""></option>
			$query = "SELECT * FROM coris_hansemerkur_zakres_ubezpieczenia  $var ORDER BY kolejnosc,nazwa";						
	...
	$result='';
	if ($tryb){
			$query = "SELECT * FROM coris_hansemerkur_biura_podrozy    WHERE ID_typ_umowy ='$typ_umowy' AND ID='$def' ORDER BY kolejnosc,nazwa";		
		...
		$result .= '<option value=""></option>';
			$query = "SELECT * FROM coris_hansemerkur_biura_podrozy WHERE ID_typ_umowy ='$typ_umowy'   ORDER BY kolejnosc,nazwa";						
    ...
    static function  umowa_dane( $case_id , $tryb ){
        ...
        $result .= '<tr><td width="60" align="right"><b><small>Program:</small></b></td><td>'.self::wysw_wariant_umowy( 'hansemerkur_program',$dane['ID_wariant'],$tryb,' style="width: 240px;" ').'</td></tr>';
        $result .= '</table>';
	...
	if ($tryb){
			$query = "SELECT ID,nazwa FROM ".self::$TABLE_program."  WHERE ID='$def' ORDER BY sort,nazwa";
			$mysql_result = mysql_query($query);
		...
		$result = '<select name="'.$name.'" id="'.$name.'" style="font-size: 8pt;width: 200px;" '.$option.'>';
			$query = "SELECT ID,nazwa FROM ".self::$TABLE_program."   ORDER BY sort,nazwa";
			$result .= '<option value=""></option>';
			$first = 1;
			$mysql_result = mysql_query($query);
			while ($row2 = mysql_fetch_array($mysql_result)) {
  ...
  static function getSumaUbezpieczenia($wariant=0){
			...
			if ($wariant > 0 ){
					$query = "SELECT * FROM coris_hansemerkur_lista_swiadczen WHERE ID_wariant='$wariant' AND ID_podlimit =0 ";
					$mysql_result = mysql_query($query);
	...
	static function aktualizujRezerweGlobalna($case_id,$rezerwa_globalna_old,$rezerwa_globalna,$waluta,$id_expenses=0,$id_claims=0){		
			$query = "UPDATE coris_hansemerkur_announce SET rezerwa_globalna = '$rezerwa_globalna',rezerwa_currency_id='$waluta' WHERE case_id ='$case_id' ";
			$mr = mysql_query($query);
			if ($mr ){
					//echo "$query <br>".mysql_error();	
					//history rezerwa_globalna
					$query = "INSERT INTO coris_hansemerkur_rezerwa_globalna_historia  SET case_id=$case_id,rezerwa_stara='$rezerwa_globalna_old',rezerwa_nowa='$rezerwa_globalna',currency_id='$waluta',ID_expenses='$id_expenses',ID_claims='$id_claims',ID_user='".$_SESSION['user_id']."',date=now();";
					$mr = mysql_query($query);		
	...
	function getRezerwaGlobalna(){
			$query = "SELECT * FROM coris_hansemerkur_announce WHERE case_id='".$this->case_id."'";
			$mysql_result = mysql_query($query);
			$row = mysql_fetch_array($mysql_result);
	...
	function listaSwiadczenZlecenie($id_swiadczenie=0){												    			
		...
		function listaSwiadczenZlecenieEdit($expense_id){												    						
				$query = "SELECT * FROM coris_hansemerkur_rezerwy WHERE ID_expenses='$expense_id'";
				$mysql_result = mysql_query($query);
				//echo $query;
				if (mysql_num_rows($mysql_result) == 0 ){
	...
	function wyswZakresUbezpieczenia($name,$def,$tryb=0,$option=''){
		...
		$query = "SELECT coris_hansemerkur_lista_swiadczen.ID,coris_hansemerkur_swiadczenia.nazwa ,coris_hansemerkur_zakres_ubezpieczenia.nazwa As zakres_nazwa 
			FROM coris_hansemerkur_lista_swiadczen,coris_hansemerkur_swiadczenia,coris_hansemerkur_zakres_ubezpieczenia		   
			WHERE coris_hansemerkur_swiadczenia.ID = coris_hansemerkur_lista_swiadczen.ID_swiadczenie
			AND coris_hansemerkur_zakres_ubezpieczenia.ID = coris_hansemerkur_lista_swiadczen.ID_zakres_ubezpieczenia
			AND (coris_hansemerkur_zakres_ubezpieczenia.ID= '".$this->rodzaj."')
			AND ( coris_hansemerkur_lista_swiadczen.ID_wariant = '".$this->wariant_umowy."' 		
			";
			
		//echo $query;	
			//$dd = count($this->opcje_umowy);
			$query .= (is_array($this->opcje_umowy) && count($this->opcje_umowy)>0) ? "OR coris_hansemerkur_lista_swiadczen.ID_wariant IN (".implode(',',$this->opcje_umowy).")" : '';
			
			$query .= ") ORDER BY 	coris_hansemerkur_lista_swiadczen.ID_zakres_ubezpieczenia,coris_hansemerkur_lista_swiadczen.kolejnosc,coris_hansemerkur_lista_swiadczen.ID  		";
		//echo $query;
	...
	static function rejestrujZmiany($case_id,$param_id,$form,$action,$zmiany){
					$qi = "INSERT coris_hansemerkur_action SET  case_id='$case_id',param_id='$param_id',form='$form',`action`='$action',ID_user='".$_SESSION['user_id']."',date=now()";														
			       	$mr = mysql_query($qi);
			       	if (!$mr ){																						
			       	...
			       	if ($id>0){
			       		foreach ($zmiany As $poz => $wart){
			       			$qi = "INSERT coris_hansemerkur_action_log SET  ID_action='$id',name='".$poz."',`table`='".$wart['table']."',`key_id`='".intval($wart['key_id'])."',`old_value`='".mysql_escape_string($wart['old'])."',new_value='".mysql_escape_string($wart['new'])."'";														
			       			$mr = mysql_query($qi);		
			       		 	if (!$mr ){																						
								echo  "<br>INSRT Error: $qi <br><br> ".mysql_error();				
							}	       			
	...
	static function getClaimsDetailsLista($case_id,$var){
		...
		$query = "SELECT cls.ID FROM coris_hansemerkur_claims cl,coris_hansemerkur_claims_details cls WHERE cl.ID_case = '$case_id' AND cl.ID=cls.ID_claims ".($var!='' ? ' AND '.$var : '')." ORDER BY cls.ID ";
		
		$mysql_result = mysql_query($query);
		$result = array();
		while ($row = mysql_fetch_array($mysql_result)){
			$result[]= new HansemerkurClaimDetails( $row['ID'] );
		}
		return $result;
	}
	...
	static function getClaims($case_id){
		...
		$query = "SELECT ID FROM coris_hansemerkur_claims   WHERE ID_case = '$case_id' ";
		$mysql_result = mysql_query($query);
		$result = array();
		while ($row = mysql_fetch_array($mysql_result)){
			$result[]= new HansemerkurClaim($row['ID'], $case_id);
		}
		return $result;
	}
	...
	static function getDecisions($case_id){
		...
		$query = "SELECT ID FROM coris_hansemerkur_decisions   WHERE ID_case = '$case_id' ";
		$mysql_result = mysql_query($query);
		
		while ($row = mysql_fetch_array($mysql_result)){
			$result[]= new HansemerkurDecision($row['ID'], $case_id);
		}
		return $result;
	}

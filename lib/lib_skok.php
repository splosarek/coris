...
Class SKOKCase{
	...
	function getCaseInfo(){
		...
		$query2 = "SELECT * FROM coris_skok_announce  WHERE case_id = '".$this->case_id."'";
	...
	function  getOpcjeUmowy(){
		$query = "SELECT ID_opcja FROM coris_skok_announce_opcje WHERE case_id = '".$this->case_id."'";
	...
	static function getCaseeuropaInfo($case_id){
		...
		$query2 = "SELECT * FROM coris_skok_announce  WHERE case_id = '$case_id'";
		$mysql_result2 = mysql_query($query2);
		$row_case_ann = mysql_fetch_array($mysql_result2);			
		return $row_case_ann;
	}
	...
	function setStatus($new_status){					
		if ( $this->case_id > 0 && $new_status>0 ){						
			$query = "UPDATE coris_skok_announce SET ID_status='$new_status' WHERE  case_id = '".$this->case_id."' LIMIT 1";
			$mysql_result = mysql_query($query);
			if ($mysql_result){
					$qu = "INSERT INTO coris_skok_status_historia SET case_id='".$this->case_id."', ID_status='$new_status',ID_user='".$_SESSION['user_id']."',date=now();";										
					$mr = mysql_query($qu);										
					if (!$mr){
	...
	static  function  wysw_status($name,$def,$tryb=0,$typ_umowy){	
		$result='';		
		if ($tryb){
				$query = "SELECT * FROM coris_skok_status_szkody    WHERE ID_typ_umowy ='$typ_umowy' AND ID='$def' ORDER BY  kolejnosc";		
	//echo $query;
				$mysql_result = mysql_query($query);
			...
			//<option value=""></option>
				$query = "SELECT * FROM coris_skok_status_szkody  WHERE ID_typ_umowy ='$typ_umowy'  ORDER BY kolejnosc";						
	...
	if ($tryb){
			$query = "SELECT * FROM coris_skok_typ_umowy   WHERE ID_client='$client_id' AND ID='$def' ORDER BY nazwa";		
		...
		$result = '<select name="'.$name.'" id="'.$name.'" style="font-size: 8pt;width: 200px;"  '.$option.'>';
		//<option value=""></option>
			$query = "SELECT * FROM coris_skok_typ_umowy WHERE ID_client='$client_id'   ORDER BY nazwa";						
		//	echo $query;
			$mysql_result = mysql_query($query);
			while ($row2 = mysql_fetch_array($mysql_result)) {
						$result .= '<option value="'. $row2['ID'] .'" '. (($row2['ID'] == $def) ? "selected" : "") .'>'.$row2['nazwa'].'</option>';
	...
	if ($tryb){
			$query = "SELECT * FROM coris_skok_zakres_ubezpieczenia    WHERE ID='$def' ORDER BY kolejnosc,nazwa";		
		...
		if ($def>0){
			...
			$qt = "SELECT * FROM coris_skok_rezerwy  WHERE case_id ='".$case_id."'";
			$mt = mysql_query($qt);
		...
		//<option value=""></option>
			$query = "SELECT * FROM coris_skok_zakres_ubezpieczenia  $var ORDER BY kolejnosc,nazwa";						
	...
	$result='';
	if ($tryb){
			$query = "SELECT * FROM coris_skok_biura_podrozy    WHERE ID_typ_umowy ='$typ_umowy' AND ID='$def' ORDER BY kolejnosc,nazwa";		
		...
		$result .= '<option value=""></option>';
			$query = "SELECT * FROM coris_skok_biura_podrozy WHERE ID_typ_umowy ='$typ_umowy'   ORDER BY kolejnosc,nazwa";						
	...
	static function  wysw_biura_podrozy_add_case($name,$def,$tryb=0,$typ_umowy,$option=''){
	...
	if ($tryb){
		...
		$result = '<select name="'.$name.'" id="'.$name.'" style="font-size: 8pt;width: 200px;"  '.$option.'>';
		$result .= '<option value=""></option>';
			$query = "SELECT * FROM coris_skok_biura_podrozy WHERE ID_typ_umowy ='$typ_umowy'  AND active=1  ORDER BY kolejnosc,nazwa";						
...
static  function  wysw_wariant_umowy($name,$def,$tryb=0,$typ_umowy,$option=''){	
	...
	if ($tryb){
			$query = "SELECT ID,nazwa FROM coris_skok_wariant_umowy   WHERE ID_typ_umowy ='$typ_umowy' AND  opcja=0 AND ID='$def' ORDER BY kolejnosc";				
			$mysql_result = mysql_query($query);
		...
		$result = '<select name="'.$name.'" id="'.$name.'" style="font-size: 8pt;width: 200px;" '.$option.'>';
			$query = "SELECT ID,nazwa FROM coris_skok_wariant_umowy   WHERE opcja=0  AND  ID_typ_umowy ='$typ_umowy' ORDER BY kolejnosc";					
...
static  function  wysw_wariant_umowy_kod($name,$def,$tryb=0,$typ_umowy,$option=''){	
	...
	if ($tryb){
			if ( !($typ_umowy > 0) )
				$typ_umowy = self::$TYP_PRODUKTU ;
			$query = "SELECT ID,nazwa,kod_taryfy FROM coris_skok_wariant_umowy   WHERE ID_typ_umowy ='$typ_umowy' AND  opcja=0 AND ID='$def' ORDER BY kolejnosc";				
			$mysql_result = mysql_query($query);
			$row2 = mysql_fetch_array($mysql_result);			
			$result = '<select name="'.$name.'" style="font-size: 8pt;width: 200px;" disabled>';					
		...
		$result = '<select name="'.$name.'" id="'.$name.'" style="font-size: 8pt;width: 200px;" '.$option.'>';
			$query = "SELECT coris_skok_wariant_umowy.ID,coris_skok_wariant_umowy.nazwa,coris_skok_wariant_umowy.kod_taryfy 
				FROM coris_skok_wariant_umowy,coris_skok_pakiet    WHERE opcja=0  
					AND  coris_skok_pakiet.ID_wariant  = coris_skok_wariant_umowy.ID
					AND coris_skok_pakiet.ID_biuro  = '".self::$BIURO_PODROZY."'
...
static  function  wysw_opcje_umowy($name,$case_id,$tryb=0,$typ_umowy,$option=''){	
	...
	if ($tryb){
			...
			$query = "SELECT ID,nazwa,coris_skok_announce_opcje.ID_opcja FROM coris_skok_wariant_umowy LEFT JOIN coris_skok_announce_opcje ON ID_opcja=coris_skok_wariant_umowy.ID AND coris_skok_announce_opcje.case_id = '$case_id'   WHERE coris_skok_wariant_umowy.opcja=1  AND  coris_skok_wariant_umowy.ID_typ_umowy ='$typ_umowy' ORDER BY kolejnosc";	
		//	echo $query;			
			$mysql_result = mysql_query($query);
			while ($row2 = mysql_fetch_array($mysql_result)) {
...
static  function  wysw_opcje_umowy_kod($name,$case_id,$tryb=0,$typ_umowy,$option=''){	
	...
	if ($tryb){
		...
		/*	if ( !($typ_umowy > 0) ){
			...
			$query = "SELECT coris_skok_wariant_umowy.ID,coris_skok_wariant_umowy.opcja_status,coris_skok_wariant_umowy.nazwa,coris_skok_announce_opcje.ID_opcja 
					FROM coris_skok_wariant_umowy LEFT JOIN coris_skok_announce_opcje ON ID_opcja=coris_skok_wariant_umowy.ID 
															AND coris_skok_announce_opcje.case_id = '$case_id'   
					WHERE
					
					 coris_skok_wariant_umowy.opcja=1  AND  coris_skok_wariant_umowy.ID_parent ='$wariant_umowy' 
					 AND coris_skok_wariant_umowy.ID_parent AND ID_parent>0
					 ORDER BY kolejnosc";	
			//echo $query;
			$mysql_result = mysql_query($query);
			while ($row2 = mysql_fetch_array($mysql_result)) {
  ...
  static function getSumaUbezpieczenia($wariant=0){
			...
			if ($wariant > 0 ){
					$query = "SELECT * FROM coris_skok_lista_swiadczen WHERE ID_wariant='$wariant' AND ID_podlimit =0 ";
					$mysql_result = mysql_query($query);
	...
	static function aktualizujRezerweGlobalna($case_id,$rezerwa_globalna_old,$rezerwa_globalna,$waluta,$id_expenses=0,$id_claims=0){		
			$query = "UPDATE coris_skok_announce SET rezerwa_globalna = '$rezerwa_globalna',rezerwa_currency_id='$waluta' WHERE case_id ='$case_id' ";
			$mr = mysql_query($query);
			if ($mr ){
					//echo "$query <br>".mysql_error();	
					//history rezerwa_globalna
					$query = "INSERT INTO coris_skok_rezerwa_globalna_historia  SET case_id=$case_id,rezerwa_stara='$rezerwa_globalna_old',rezerwa_nowa='$rezerwa_globalna',currency_id='$waluta',ID_expenses='$id_expenses',ID_claims='$id_claims',ID_user='".$_SESSION['user_id']."',date=now();";
					$mr = mysql_query($query);		
	...
	function getRezerwaGlobalna(){
			$query = "SELECT * FROM coris_skok_announce WHERE case_id='".$this->case_id."'";
			$mysql_result = mysql_query($query);
			$row = mysql_fetch_array($mysql_result);
	...
	function listaSwiadczenZlecenie($id_swiadczenie=0){	
		...
		$result = '
				...
				<td align="left" colspan="1">'. $this->wyswZakresUbezpieczenia('id_swiadczenie',$id_swiadczenie,0,'class="required" onChange="skok_getSumaUbezp_do_wyk('.$this->case_id.',this.value,\'suma_ubezpieczenia\',\'rezerwa_globalna\');"').'				
				</tr>
				<tr> 		                
                <td align="right">';
		...
		function listaSwiadczenZlecenieEdit($expense_id){												    						
				...
				$rezerwa_globalna = '';
		$result = '		
				<tr><td  align="right"><small><b>Åšwiadczenie: </b></small></td>
	...
	function wyswZakresUbezpieczenia($name,$def,$tryb=0,$option=''){
		...
		$query = "SELECT coris_skok_lista_swiadczen.ID,coris_skok_swiadczenia.nazwa ,coris_skok_zakres_ubezpieczenia.nazwa As zakres_nazwa 
			FROM coris_skok_lista_swiadczen,coris_skok_swiadczenia,coris_skok_zakres_ubezpieczenia		   
			WHERE coris_skok_swiadczenia.ID = coris_skok_lista_swiadczen.ID_swiadczenie
			AND coris_skok_zakres_ubezpieczenia.ID = coris_skok_lista_swiadczen.ID_zakres_ubezpieczenia
			AND (coris_skok_zakres_ubezpieczenia.ID= '".$this->rodzaj."')
			AND ( coris_skok_lista_swiadczen.ID_wariant = '".$this->wariant_umowy."' 		
			";
			
			
			//$dd = count($this->opcje_umowy);
			$query .= (is_array($this->opcje_umowy) && count($this->opcje_umowy)>0) ? "OR coris_skok_lista_swiadczen.ID_wariant IN (".implode(',',$this->opcje_umowy).")" : '';
			
			$query .= ") ORDER BY 	coris_skok_lista_swiadczen.ID_zakres_ubezpieczenia,coris_skok_lista_swiadczen.kolejnosc,coris_skok_lista_swiadczen.ID  		";
		//echo $query;

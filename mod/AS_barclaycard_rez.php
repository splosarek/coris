	...
	$message = '';
	 ...
	 if (isset($change['ch_rezerwa_zgloszenie']) && $case_id > 0  ){		
		...
		if ($res[0]){					
			...
			$barclaycard_program= getValue('barclaycard_program');
			...
			$var = " ID_typ_umowy='$typ_umowy', ID_wariant='$barclaycard_program' ";
/////////////////////						
			$qt = "SELECt case_id FROM coris_barclaycard_announce  WHERE case_id='$case_id'";
			$mt = mysql_query($qt);			
												
			if (mysql_num_rows($mt)==0){
				$query = "INSERT INTO coris_barclaycard_announce SET case_id='$case_id', $var ";
								
			}else{
				$query = "UPDATE coris_barclaycard_announce SET $var  WHERE case_id='$case_id' LIMIT 1";				
			}						
			...
			$europa_case = new BarclaycardCase($case_id);
		...
		if ($res[0]){									
			...
			if ($rezerwa_globalna != $rezerwa_globalna_old || $rezerwa_currency_id != $rezerwa_currency_id_old){						
					BarclaycardCase::aktualizujRezerweGlobalna($case_id,$rezerwa_globalna_old,$rezerwa_globalna,$rezerwa_currency_id);		
			}			
		}else{//error update
			echo $res[1];			
		}					
...
if ($row_case_settings['client_id'] == 17241 ){
	...
	<br><br><br><br><br><div align="center"><b>Tylko dla spraw TU BARLEYCARD</b></div>
	</div>
	';
...
if (isset($change['rezerwa_zgloszenie'])){
	...
	<script language="JavaScript1.2">
		...
		function validate() {
			...
			//alert (document.getElementById(\'barclaycard_program\').length);
			if (document.getElementById(\'barclaycard_program\').value > 0 || document.getElementById(\'barclaycard_program\').length < 2) {
			
			}else{
				alert("Prosze wybraæ wariant umowy");
				document.getElementById(\'barclaycard_program\').focus();
				return false;
			}			*/
			return true;
		}												
		...
		$result .= '<table cellpadding="5" cellspacing="0" border="0" bgcolor="#e0e0e0" width="100%" >
			...
			<tr bgcolor="#AAAAAA">	<td width="5%">&nbsp;</td>
					<td  align="right">
					<b>Program:</b>	</td><td>';
					//$result .= BarclaycardCase::wysw_typy_umowy($row['client_id'],'typ_umowy',$row2['ID_typ_umowy'],0);
                    $result .= BarclaycardCase::wysw_wariant_umowy( 'barclaycard_program',$row2['ID_wariant'],0,' style="width: 240px;" ');
					$result .= '
					</td>
			</tr>';
				...
				$result .= '<tr bgcolor="#AAAAAA">							
					...
					<td  align="right">
					<b>Status:</b></td><td> ';									
					 $result .= BarclaycardCase::wysw_status('status_szkody',$row2['ID_status'],0);
							$result .= '
						</td>
					</tr>';						
			  $result .= '<tr>	
		...
		$result .= '<table cellpadding="5" cellspacing="0" border="0" bgcolor="#e0e0e0" width="100%" >
				...
				//$result .= BarclaycardCase::wysw_typy_umowy($row['client_id'],'typ_umowy',$row2['ID_typ_umowy'],1,'onChange="getWariantUmowy(this.value,\'barclaycard_program\');"');
                    $result .= BarclaycardCase::wysw_wariant_umowy( 'barclaycard_program',$row2['ID_wariant'],1,' style="width: 240px;" ');
					$result .= '
								</td></tr>';
			  	...
			  	$result .= '<tr bgcolor="#AAAAAA">							
					...
					<td  align="right">
					<b>Status:</b></td><td> ';									
					 $result .= BarclaycardCase::wysw_status('status_szkody',$row2['ID_status'],1);
							$result .= '
						</td>
					</tr>						
			  <tr>		

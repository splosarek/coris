	...
	if (isset($change['ch_claims_zgloszenia']) && $case_id > 0  ){		
		...
		if ($res[0]){			   	
			...
			//	$add_amount_pln = str_replace(',','.',getValue('amount_pln'));
				...
				$query  = "INSERT INTO coris_assistance_cases_claims  SET ID_case ='$case_id',
				...
				if ($poz>0){// insert pozycje
					...
					$tmp = isset( $_POST['add_zgloszenie_opis_roszczenia'] ) ?  $_POST['add_zgloszenie_opis_roszczenia']  : null ;
						...
						if (is_array($tmp)){
								foreach ( $tmp As   $key => $val ){									
										...
										if ($add_zgloszenie_pozycja_usun != 1 && $add_amount_pln>0){
											$qu  = "INSERT INTO coris_assistance_cases_claims_details   SET  ID_claims  ='$poz',amount ='$zgloszenie_kwota',amount_accept ='$zgloszenie_kwota',
						...
						$query  = "UPDATE coris_assistance_cases_claims  SET announcer ='$announcer',ID_institution='$zgloszenie_id_instytucja',announce_date='$data_zgloszenia',note='$zgloszenie_uwagi' WHERE ID='$claims_id'";		
						$mysql_result = mysql_query($query);
						$poz=0;
						if ($mysql_result){
							//$message .= "Udpate OK ".$query;							
						...
						if (is_array($tmp)){
								foreach ( $tmp As   $key => $val ){
									...
									if ($zgloszenie_pozycja_usun){
										$qt = "SELECT ID FROm coris_assistance_cases_claims_pay_position WHERE ID_claims_details = '".$key."'";
										$mt = mysql_query($qt);
										if (mysql_num_rows($mt) == 0 ){																					
											$del = "DELETE FROM coris_assistance_cases_claims_details   WHERE ID_claims='$claims_id' AND ID='$key' AND status=0 LIMIT 1";											
											$md = mysql_query($del);
										}									
									}else{
										...
										$qu = "UPDATE coris_assistance_cases_claims_details   SET   amount ='$zgloszenie_kwota',currency_id ='$zgloszenie_currency_id',
				...
				if(getValue('edit_form_action') == 'pay_add'){
					...
					if ( $roszczenie>0 && is_array($roszczenie_pozycja) ){
						...
						<b>¦wiadczenia:</b>
							...
							$query = "SELECT coris_assistance_cases_claims_details.*,
							(SELECT nazwa FROM coris_signal_ryzyko_operat WHERE coris_signal_ryzyko_operat.ID=coris_assistance_cases_claims_details.ID_operat  ) As operat, 
							(SELECT initials  FROM coris_users  WHERE coris_users.user_id =coris_assistance_cases_claims_details.ID_user ) As user,
							(SELECT initials  FROM coris_users  WHERE coris_users.user_id =coris_assistance_cases_claims_details.status_ID_user ) As status_user
...
function  wysw_instytucja2($name,$def,$tryb=0){
	if ($tryb){
		if ($def> 0){
			$query = "SELECT kod,nazwa,miasto FROM coris_signal_institution2   WHERE kod='$def' ORDER BY nazwa,miasto";									
			$mysql_result = mysql_query($query);			
		...
		$result = '<select name="'.$name.'"  id="'.$name.'" style="font-size: 7pt;">
					<option value=""></option>';
			$query = "SELECT kod,nazwa,miasto FROM coris_signal_institution2 ORDER BY nazwa,miasto";						
			$mysql_result = mysql_query($query);

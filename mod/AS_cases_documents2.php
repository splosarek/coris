...
function module_main()
{
    ...
    $result .= '<div style="width: 1310px;border: #6699cc 1px solid;height:auto; background-color: #DFDFDF;;margin: 5px;">';
    $result .= dokumenty('', 0);
    $result .= '</div>';
...
function dokumenty($row, $tryb)
{
    ...
    $result .= '  <script language="javascript">
			    ...
			    function zastosuj_filter(){
			    	if ($(\'filter_documents\')){
						typ = $(\'filter_documents\').value
						cat = $(\'filter_category_id\').value
						txt = $(\'search_txt\').value
						s = $(\'order\').value
						sd = $(\'sort_direction\').value
						
						init_documents(' . $case_id . ',\'type=\'+typ+\'&category=\'+cat+\'&txt_search=\'+txt+\'&order=\'+s+\'&sort_direction=\'+sd)
					}
				}

...
$accesss_change_liquidation = array(275,85,26,256,39,159,76);
...
function module_main(){
	...
	$result = '';
			...
			$query = "SELECT ac.number, ac.year, ac.client_id, ac.type_id, ac.genre_id, ac.paxname, ac.paxsurname, ac.watch, ac.ambulatory, ac.hospitalization, 
		                 ac.transport, ac.decease, ac.costless,ac.only_info, ac.costless, ac.unhandled, ac.archive,ac.archive_date,ac.archive_user_id, ac.archive0,ac.archive0_date,ac.archive0_user_id, ac.reclamation,ac.fraud,ac.fraud_date,ac.fraud_user_id, ac.status_client_notified,
...
if (isset($change['settings_status2'])){
		...
		<table cellpadding="1" cellspacing="0" border="1" align="center" width=95%>
		 <tr>
			<td width="25%" align="center" '. (($row['attention2']) ? "bgcolor=\"#6699cc\" style=\"color: yellow\"" : "style=\"color: red\"").'>
			...
			<td width="25%" align="center" '. (($row['attention']) ? "bgcolor=\"red\" style=\"color: yellow\"" : "style=\"color: red\"").'>
			...
			<td width="25%" align="center" '. (($row['reclamation']) ? "bgcolor=\"red\" style=\"color: yellow\"" : "style=\"color: red\"").'>
				<input type="checkbox"   value="1" name="reclamation"  id="reclamation" style="background: red;" '. (($row['reclamation']) ? "checked" : "").'>&nbsp;<font onclick="FormcheckboxSelect(\'reclamation\');" style="cursor: default" title="'. AS_TITLE_REKLAM1 .'"><b>'. AS_TITLE_REKLAM1 .'</b></font>
			 </td>																
	...
	<table cellpadding="1" cellspacing="0" border="1" align="center" width=95%>
		<tr>
			...
			<td width="25%" align="center" '. (($row['reclamation']) ? "bgcolor=\"red\" style=\"color: yellow\"" : "style=\"color: red\"").'>
				<input type="checkbox"  disabled value="1" name="reclamation"  id="reclamation" style="background: red;" '. (($row['reclamation']) ? "checked" : "").'>&nbsp;<font style="cursor: default"  title="'. AS_TITLE_REKLAM1 .'"><b>'. AS_TITLE_REKLAM1 .'</b></font>
			</td>				
			<td width="25%" align="center" >
	...
	<script>
	function click_status_assist_complete(obj){

					if ( obj.checked == true ){
						ayax_action=1;
					var url = \'ayax/case_check_new_documents.php\';
					var jsonRequest = new Request.JSON({url: url, encoding: \'UTF-8\' ,
					onComplete: function(jsonObj) {
						ilosc = jsonObj.ilosc;

						if (ilosc > 0){
							alert(\'Uwaga W sprawie s± jeszcze nieobrobione dokumenty!!!\');
							obj.checked  = false;
							return false;
						}else{
	...
	function click_status_assist_complete2(){

					if ( $(\'status_assist_complete\').checked == false ){
						ayax_action=1;
					var url = \'ayax/case_check_new_documents.php\';
					var jsonRequest = new Request.JSON({url: url, encoding: \'UTF-8\' ,
					onComplete: function(jsonObj) {
						...
						if (ilosc > 0){
							alert(\'Uwaga W sprawie s± jeszcze nieobrobione dokumenty!!!\');
							$(\'status_assist_complete\').checked  = false;
							return false;
						}else{
								$(\'status_assist_complete\').checked = true;
								return true;
						}
...
if (isset($change['settings_status'])){
		...
		<td width="11%"  nowrap title="'.AS_CASD_ZAZNZOSPOTWWAZN.'">
					<input disabled  type="checkbox" id="status_policy_confirmed" value="1" name="status_policy_confirmed" style="background: #dfdfdf" '. (($row['status_policy_confirmed']) ? "checked disabled" : "").' title="'.($row['status_policy_confirmed']==1 ? 'Data: '.$row['status_policy_confirmed_date']."\n"."U¿ytkownik: ".getUserName($row['status_policy_confirmed_user_id']) : '' ).'">&nbsp;<font style="cursor: default"  >'. AS_CASD_POTWPOL .'</font>
		...
		<td  nowrap nowrap title="BACK-OFFICE zakoñczona" width="120">
      ...
      <td  nowrap nowrap title="'.AS_CASD_ZAZLIQUIDSTOP.'" width="120">
		...
		<td nowrap title="'.AS_CASD_ZAZNZAKDZASST.'" width="120">
					<div style="float:left;width:20px;"><input onCLick="click_status_assist_complete(this);" type="checkbox" id="status_assist_complete" value="1" name="status_assist_complete" style="background: #dfdfdf" '. (($row['status_assist_complete']) ? "checked" : "").' title="'.($row['status_assist_complete']==1 ? 'Data: '.$row['status_assist_complete_date']."\n"."U¿ytkownik: ".getUserName($row['status_assist_complete_user_id']) : '' ).'">
					<input type="hidden"   id="status_assist_complete_old" name="status_assist_complete_old" value="'. $row['status_assist_complete'].'">
					</div>
					<div style="float:left;width:90px;text-align:center;">&nbsp;<font style="cursor: default"   onclick="click_status_assist_complete2();" color="green"><b>'. AS_CASD_ZAKONCZ .'</b></font></div>
	...
	<table cellpadding="1" cellspacing="0" border="1" align="center" width=95%>
		...
		<td  nowrap title="BACK-OFFICE zakoñczona" width="120">

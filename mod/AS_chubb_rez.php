<?php
include_once 'lib/lib_chubb.php';
	...
	$message = '';
	 ...
	 if (isset($change['ch_rezerwa_zgloszenie']) && $case_id > 0  ){		
		...
		if ($res[0]){					
			...
			$chubb_program= getValue('chubb_program');
			...
			$var = " ID_typ_umowy='$typ_umowy', ID_wariant='$chubb_program' ";
/////////////////////						
			$qt = "SELECt case_id FROM `".ChubbCase::$TABLE_ANNONUNCE."`  WHERE case_id='$case_id'";
			$mt = mysql_query($qt);			
												
			if (mysql_num_rows($mt)==0){
				$query = "INSERT INTO `".ChubbCase::$TABLE_ANNONUNCE."` SET case_id='$case_id', $var ";
								
			}else{
				$query = "UPDATE `".ChubbCase::$TABLE_ANNONUNCE."` SET $var  WHERE case_id='$case_id' LIMIT 1";				
			}						
			...
			$europa_case = new chubbCase($case_id);
		...
		if ($res[0]){									
			...
			if ($rezerwa_globalna != $rezerwa_globalna_old || $rezerwa_currency_id != $rezerwa_currency_id_old){						
					ChubbCase::aktualizujRezerweGlobalna($case_id,$rezerwa_globalna_old,$rezerwa_globalna,$rezerwa_currency_id);		
			}			
		}else{//error update
			echo $res[1];			
		}					
...
function module_main(){
	...
	$result = '';	
		...
		$query2 = "SELECT * FROM `".ChubbCase::$TABLE_ANNONUNCE."`  WHERE case_id = '$case_id'";
		$mysql_result2 = mysql_query($query2);
		$row_case_ann = mysql_fetch_array($mysql_result2);			
		
if ($row_case_settings['client_id'] == ChubbCase::$CLIENT_ID ){
	...
	<br><br><br><br><br><div align="center"><b>Tylko dla spraw CHUBB</b></div>
	</div>
	';
...
if (isset($change['rezerwa_zgloszenie'])){
	...
	<script language="JavaScript1.2">
		...
		function validate() {
			...
			//alert (document.getElementById(\'chubb_program\').length);
			if (document.getElementById(\'chubb_program\').value > 0 || document.getElementById(\'chubb_program\').length < 2) {
			
			}else{
				alert("Prosze wybraæ wariant umowy");
				document.getElementById(\'chubb_program\').focus();
				return false;
			}			*/
			return true;
		}												
	...
	/*
	  	<tr bgcolor="#AAAAAA">	<td width="5%">&nbsp;</td>
					<td  align="right">
					<b>Program:</b>	</td><td>';
					//$result .= chubbCase::wysw_typy_umowy($row['client_id'],'typ_umowy',$row2['ID_typ_umowy'],0);
                    $result .= chubbCase::wysw_wariant_umowy( 'chubb_program',$row2['ID_wariant'],0,' style="width: 240px;" ');
					$result .= '
					</td>
			</tr>';
	*/
		$result .= '<table cellpadding="5" cellspacing="0" border="0" bgcolor="#e0e0e0" width="100%" >';
				...
				$result .= '<tr bgcolor="#AAAAAA">							
					...
					<td  align="right">
					<b>Status:</b></td><td> ';									
					 $result .= chubbCase::wysw_status('status_szkody',$row2['ID_status'],0);
							$result .= '
						</td>
					</tr>';						
			  $result .= '<tr>	
    ...
    //$result .= chubbCase::wysw_typy_umowy($row['client_id'],'typ_umowy',$row2['ID_typ_umowy'],1,'onChange="getWariantUmowy(this.value,\'chubb_program\');"');
    $result .= chubbCase::wysw_wariant_umowy( 'chubb_program',$row2['ID_wariant'],1,' style="width: 240px;" ');
    $result .= '
    </td></tr>
     */
		$result .= '<table cellpadding="5" cellspacing="0" border="0" bgcolor="#e0e0e0" width="100%" >
			...
			';
			  	...
			  	$result .= '<tr bgcolor="#AAAAAA">							
					...
					<td  align="right">
					<b>Status:</b></td><td> ';									
					 $result .= chubbCase::wysw_status('status_szkody',$row2['ID_status'],1);
							$result .= '
						</td>
					</tr>						
			  <tr>		
...
if (isset($change['rezerwy_rezerwy'])){
		...
		<tr bgcolor="#AAAAAA"><td width="70%" align="right"><b>Rezerwa globalna:</b></td><td width="30%">
			...
			<input type="hidden" name="rezerwa_globalna_old" id="rezerwa_globalna_old" value="'.print_currency($row_case_ann['rezerwa_globalna']).'">';   
				...
				$result .= wysw_currency_eur('rezerwa_currency_id',$row_case_ann['rezerwa_currency_id'],0,'  ');
...
$zakres = '';
		...
		<tr bgcolor="#AAAAAA"><td width="70%" align="right"><b>Rezerwa globalna:</b></td><td width="30%">
			<input type="text" name="rezerwa_globalna" id="rezerwa_globalna" value="'.print_currency($row_case_ann['rezerwa_globalna']).'"  style="text-align: right;" size="15" maxlength="20" disabled >				';   
				...
				$result .= wysw_currency_eur('rezerwa_currency_id',$row_case_ann['rezerwa_currency_id'],0,' disabled ');

...
//	return ;
    ...
    if (isset($change['ch_claims_wyplaty']) && $case_id > 0  ) {
        ...
        if ($edit_form_action == 'invoice_add'  && $edit_form_action_val > 0){
            ...
            $query = "SELECT * FROM ".GothaerCase::$TABLE_PAYMENT." WHERE ID='$edit_form_action_val'";
            ...
            if (mysql_num_rows($ms) > 0 ){
                ...
                if ($inv_id > 0 ){
                    $query = "UPDATE ".GothaerCase::$TABLE_PAYMENT." SET 	ID_invoice_in='$inv_id', status=1  WHERE ID='$payment_id'";
                    $ms = mysql_query($query);
		...
		if ($res[0]){			   	
			...
			if ($edit_form_action == 'claims_add'){	
				...
				$cl = new GothaerClaim(0,$case_id,1);
				$cl->setAnnounce_date($data_zgloszenia);
				$cl->setNote($zgloszenie_uwagi);

				$cls = new GothaerClaimDetails(0, 0,1);
			...
			/*if($zgloszenie_kwota_zaakceptowana > 0.00 || $zgloszenie_pozycja_status==4){							
				...
				$cl = new GothaerClaim($claims_id,$case_id);
				$cl->setAnnounce_date($data_zgloszenia);
				$cl->setNote($zgloszenie_uwagi);
				...
				foreach ($cl->getClaimDetails() As $pozycjaCL  ){
					...
					if ($cofnij_akceptacje==1 ){  // wycofanie akceptacji //&& compensaCase::isAdmin()
				...
				if (is_array($tmp)){
					foreach ( $tmp As   $key => $val ){									
						...
						if ($add_zgloszenie_pozycja_usun != 1 ){//&& $zgloszenie_kwota>0
								$cls = new GothaerClaimDetails(0, $claims_id,1);
								$cls->setKwota_roszczenia($zgloszenie_kwota);
								$cls->setWaluta($zgloszenie_waluta);
								if ( $zgloszenie_waluta == 'PLN' ){
										$cls->setKwota_rezerwa($zgloszenie_kwota); 	
			...
			if ($edit_form_action=='decissions_do_poprawy'){						
				if (is_array($decyzja_lista_pozycji)){
					foreach ($decyzja_lista_pozycji As $poz ){						
						$poz = intval($poz);
						if ($poz>0){
							$cls = new GothaerClaimDetails($poz);
							$cls->setStatus2(2);
							$cls->zmienStatus2();						
						}
					}
				...
				if (is_array($decyzja_lista_pozycji)){
						...
						GothaerDecision::createDecision($case_id,$lista);
					...
					$tekst6 = getValue('tekst6');
					$data_decyzji = getValue('data_decyzji');

					GothaerDecision::updateDecision($decisions_id, $tekst1, $tekst2, $tekst3, $tekst4, $tekst5, $tekst6,$data_decyzji);
		...
		if ($res[0]){			   	
			...
			$row_case_ann =GothaerCase::getCaseInfo($case_id);
			...
			if (count($update) > 0 ){
										$query = "UPDATE ".GothaerCase::$TABLE_ANNONUNCE." SET 
										".implode(', ', $update)."
			...
			if (count($update_roznice) > 0 ){
				GothaerCase::rejestrujZmiany($case_id,$case_id,'Roszczenie','UPDATE',$update_roznice);
			}							
		}
	}
	echo $message;	
...
if ($row_case_settings['client_id'] == 496  ){
	...
	<br><br><br><br><br><div align="center"><b>Tylko dla spraw TU Gothaer</b></div>
	</div>
	';
...
if (isset($change['claims_zgloszenia'])){
			...
			';
			$lista = GothaerCase::getClaims($case_id);
...
if ( $checkRezerwy ){
		...
		<tr bgcolor="#AAAAAA"><td width="70%" align="right"><b>Rezerwa globalna:</b></td><td width="30%">
			<input type="hidden" name="case_rezerwa_globalna_zmiana" id="case_rezerwa_globalna_zmiana" value="1"> 
			<input type="text" name="rezerwa_globalna" id="rezerwa_globalna" value="'.print_currency($row_case_gr['rezerwa_globalna']).'"  style="text-align: right;" size="15" maxlength="20"  >		
			...
			<input type="hidden" name="rezerwa_claims_id" id="rezerwa_claims_id" value="0">';   
				...
				$result .= wysw_currency_eur('rezerwa_currency_id','EUR',0,' class="disabled" ');
			...
			/*$result .= '<tr bgcolor="#AAAAAA"><td width="70%" align="right"><b>Rezerwa do wykorzystania:</b></td><td width="30%">
		...
		$result .= '</table><br>';
			...
			if (getValue('edit_form_action') == 'claims_edit' && getValue('edit_form_action_param') > 0  ){
				...
				$cl = new GothaerClaim($edit_cl, $case_id);
...
if ( $checkRezerwy ){
		...
		<tr bgcolor="#AAAAAA"><td width="70%" align="right"><b>Rezerwa globalna:</b></td><td width="30%">
			...
			<input type="hidden" name="rezerwa_claims_id" id="rezerwa_claims_id" value="0">';   
				...
				$result .= wysw_currency_eur('rezerwa_currency_id','EUR',1,' class="disabled" ');
				$result .= '	<input type="hidden" name="rezerwa_currency_id_old" id="rezerwa_currency_id_old" value="'.print_currency($row_case_gr['currency_id']).'">';
...
if (isset($change['claims_decyzje'])){
			...
			function probny_wydruk(){
					...
					popup(\'AS_cases_gothaer_claims_print_test.php?lista=\'+pozycje,\'wydruk\',1000,800);
						
			}	
</script>
			';
			$lista = GothaerCase::getDecisions($case_id);		
						$result .= '
		    <table cellpadding="1" cellspacing="0" border="1" align="center" width=90%>
		      <tr bgcolor="#999999">
			...
			foreach ($lista As $pozycja  ){
				...
				if (getValue('edit_form_action') == 'decissions_edit' && getValue('edit_form_action_param') == $cl->getID()  ){
						...
						<tr>		  
								<td width="70" align="center"><b>Status</td>						  						  							
						  		...
						  		<td width="90" align="center"><b>Kwota zaakcept.</b></td>		
						  		<td width="90" align="center"><b>Kwota w PLN</b></td>		
						  ...
						  ';
								...
								foreach ($lista_pozycji As $poz){
									$result .= '<tr>';
										...
										$cls = new GothaerClaimDetails($poz['ID_claims_details']);
										
										$result .= '<td align="left"> '.$cls->getNote().'  </td>';	
										$result .= '<td align="right"> '.Finance::print_currency($poz['amount']).' '.$poz['currency_id'].'</td>';	
										$result .= '<td align="right"> '.Finance::print_currency($poz['payment_amount']).' PLN</td>';	
						...
						<tr bgcolor="#BBBBBB"><td width="10" rowspan=3 valign="top">&nbsp;</td>																			
							<td>														
							<div style="width:49%;float:left;"><b>Adresat decyzji:</b><br> <textarea cols="50" rows="3" id="tekst5" name="tekst5">'.$adresat.'</textarea>
							</div>
							<div style="width:49%;float:right;"><b>Beneficjent :</b><br> <textarea cols="50" rows="3" id="tekst4" name="tekst4">'.$beneficjent.'</textarea>
							</div>
							<div style="clear: both"></div>
							<br><br>
							<b>Typ szkody:</b><br> <textarea cols="110" rows="2" id="tekst6" name="tekst6">'.$txt6.'</textarea>
							
							<br><br>
							<b>'.AS_CASES_DEC.' - nag³ówek:</b><br> <textarea cols="110" rows="8" id="tekst1" name="tekst1">'.$txt1.'</textarea>
							<br><br>
							<b>'.AS_CASES_DEC.' - uzasadnienie:</b><br> <textarea cols="110" rows="8" id="tekst2" name="tekst2">'.$txt2.'</textarea>							
							
							<br><br>
							<b>Operat - sumy ubezpieczenia (tylko decyzja wyp³ata):</b><br> 
				...
				$lista = GothaerCase::getClaimsDetailsLista($case_id,'status2=1');		
						$result .= '
		   			 <table cellpadding="1" cellspacing="0" border="1" align="center" width=90%>
		      				<tr bgcolor="#999999">
		      					<td width="5%" align="center">&nbsp;</td>		      								
		  ...
		  $lista = GothaerCase::getDecisions($case_id);		
			...
			foreach ($lista As $pozycja  ){
				...
				$result .= '<tr bgcolor="#BBBBBB">';
					...
					$result .= '<td align="center">'.$cl->getStatusName().' '.($cl->getStatus()==1 ? '<a href="AS_cases_gothaer_claims_print.php?id='.$cl->getId().'" target="_blank"><img border="0" src="img/print.gif"></a>' : '' ).'</td>';
					$result .= '<td align="center">'.$cl->getTypeName().'</td>';													
					$result .= '<td>'.$cl->getDate().'</td>';																														
					$result .= '<td align="right">'.print_currency($cl->getPayment_amount()).' '.$cl->getCurrency_id().'</td>';																									
					$result .= '<td>'.Application::getUserName($cl->getID_user()).'</td>';												
...
//	return $result;
	...
	if (isset($change['claims_wyplaty'])){
			 ...
			 $query = "SELECT * FROM ".GothaerCase::$TABLE_PAYMENT." WHERE ID_case= '$case_id'";
			 $mysql_result = mysql_query($query);		
						$result .= '
		    <table cellpadding="1" cellspacing="0" border="1" align="center" width=90%>
		      <tr bgcolor="#999999">
			...
			while($row = mysql_fetch_array($mysql_result)){								 		
				$result .= '<tr bgcolor="#BBBBBB">';
					...
					$result .= '<td align="right">'.print_currency($row['amount']).' PLN</td>';													

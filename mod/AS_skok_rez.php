	 ...
	 if (isset($change['ch_rezerwa_zgloszenie']) && $case_id > 0  ){		
		...
		if ($res[0]){					
			...
			if (is_array($opcje_ubezpieczenia)){
				$query = "DELETE  FROM coris_skok_announce_opcje WHERE case_id ='$case_id' AND ID_opcja NOT IN (".implode(',',$opcje_ubezpieczenia).")  ";
			}else{
					$query = "DELETE  FROM coris_skok_announce_opcje WHERE case_id ='$case_id'  ";
			}
...
/////////////////////						
			$qt = "SELECt case_id FROM coris_skok_announce  WHERE case_id='$case_id'";
			$mt = mysql_query($qt);			
			...
			if (mysql_num_rows($mt)==0){
				...
				$query = "UPDATE coris_skok_announce SET $var  WHERE case_id='$case_id' LIMIT 1";				
			}						
			...
			if ($status_szkody != $old_status_szkody)
					$skok_case->setStatus($status_szkody);		
		}else{//error update
			echo $res[1];
		...
		if ($res[0]){									
			...
			if ($rezerwa_globalna != $rezerwa_globalna_old || $rezerwa_currency_id != $rezerwa_currency_id_old){						
					skokCase::aktualizujRezerweGlobalna($case_id,$rezerwa_globalna_old,$rezerwa_globalna,$rezerwa_currency_id);		
			}			
		}else{//error update
			echo $res[1];			
		}					
...
if ( $row_case_settings['client_id'] == 10){
	...
	<br><br><br><br><br><div align="center"><b>Tylko dla spraw TU skok</b></div>
	</div>
	';
	...
	<script language="JavaScript1.2">
		...
		$result .= '<table cellpadding="5" cellspacing="0" border="0" bgcolor="#e0e0e0" width="100%" >
			...
			<tr bgcolor="#AAAAAA">	<td width="5%">&nbsp;</td>
					<td  align="right">
					<b>Typ umowy:</b>	</td><td>';
					$result .= SKOKCase::wysw_typy_umowy($row['client_id'],'typ_umowy',$row2['ID_typ_umowy'],0);
					$result .= '
					</td>
			</tr>';
			if ($row['client_id']==10){		
				$result .= '<tr bgcolor="#AAAAAA">	<td width="5%">&nbsp;</td>
					...
					$result .= SKOKCase::wysw_biura_podrozy('biuro_podrozy',$row2['ID_biuro_podrozy'],0,$row2['ID_typ_umowy'],'onChange="getWariantUmowySkokKod(this.value,document.getElementById(\'typ_umowy\').value,\'wariant_ubezpieczenia\',\'opcje_ubezpieczenia\');"');			
						$result .= '
					</td>
					</tr>';
			...
			$result .= '<tr bgcolor="#AAAAAA">							
					...
					<b>Wariant umowy:</b></td><td> ';									
					 $result .= SKOKCase::wysw_wariant_umowy_kod('wariant_ubezpieczenia',$row2['ID_wariant'],0,$row2['ID_typ_umowy'],'onChange="getWariantUmowySkokKodOpcje(this.value,\'opcje_ubezpieczenia\');"');	
							$result .= '
						</td>
					...
					<td  align="right">
					<b>Opcje:</b></td><td> ';									
					 $result .= SKOKCase::wysw_opcje_umowy_kod('opcje_ubezpieczenia',$row2['case_id'],0,$row2['ID_typ_umowy']);	
							$result .= '
						</td>
					</tr>';
			$result .= '<tr bgcolor="#AAAAAA">							
					...
					<b>Zakres ubezpieczenia:</b></td><td> ';									
					 $result .= SKOKCase::wysw_rodzaj_szkody($row2['case_id'],'rodzaj_szkody',$row2['ID_rodzaj'],0,$row2['ID_typ_umowy']);	
...
/*				$result .= '<tr bgcolor="#AAAAAA">							
					...
					<b>Sprawa zale¿na:</b></td><td> ';									
					 //$result .= SkokCase::wysw_rodzaj_szkody('rodzaj_szkody',$row2['ID_rodzaj'],0,$row2['ID_typ_umowy']);	
...
*/			
				...
				$result .= '<tr bgcolor="#AAAAAA">							
					...
					<td  align="right">
					<b>Status:</b></td><td> ';									
					 $result .= SKOKCase::wysw_status('status_szkody',$row2['ID_status'],0,$row2['ID_typ_umowy']);	
							$result .= '
						</td>
					</tr>';						
			  $result .= '<tr>	
		...
		$result .= '<table cellpadding="5" cellspacing="0" border="0" bgcolor="#e0e0e0" width="100%" >
				...
				$result .= SKOKCase::wysw_typy_umowy($row['client_id'],'typ_umowy',$row2['ID_typ_umowy'],1,'onChange="getWariantUmowy(this.value,\'wariant_ubezpieczenia\');"');
					$result .= '
								</td></tr>';
			...
			if ($row['client_id']==10){		
				$result .= '<tr bgcolor="#AAAAAA">	<td width="5%">&nbsp;</td>
					<td align="right">
					<b>Biuro podró¿y:</b>	</td><td>';
					$result .= SKOKCase::wysw_biura_podrozy('biuro_podrozy',$row2['ID_biuro_podrozy'],1,$row2['ID_typ_umowy'],'');			
						$result .= '
					</td>
					</tr>';
			...
			$result .= '<tr bgcolor="#AAAAAA">							
					...
					<td  align="right">
					<b>Wariant umowy:</b></td><td> ';									
					 $result .= SKOKCase::wysw_wariant_umowy_kod('wariant_ubezpieczenia',$row2['ID_wariant'],1,$row2['ID_typ_umowy']);	
							$result .= '
						</td>
					</tr><tr bgcolor="#AAAAAA">							
					<td width="5%">&nbsp;</td>
					<td  align="right">
					<b>Opcje:</b></td><td> ';									
					 $result .= SKOKCase::wysw_opcje_umowy_kod('opcje_ubezpieczenia',$row2['case_id'],1,$row2['ID_typ_umowy']);	
							$result .= '
						</td>
					</tr>';
								$result .= '<tr bgcolor="#AAAAAA">							
					...
					<td  align="right">
					<b>Zakres ubezpieczenia:</b></td><td> ';									
					 $result .= SKOKCase::wysw_rodzaj_szkody($row2['case_id'],'rodzaj_szkody',$row2['ID_rodzaj'],1,$row2['ID_typ_umowy']);	
							$result .= '
						</td>
					</tr>';	
			...
			/*	$result .= '<tr bgcolor="#AAAAAA">							
			  	...
			  	$result .= '<tr bgcolor="#AAAAAA">							
					...
					<td  align="right">
					<b>Status:</b></td><td> ';									
					 $result .= SKOKCase::wysw_status('status_szkody',$row2['ID_status'],1,$row2['ID_typ_umowy']);	
							$result .= '
						</td>
					</tr>						
			  <tr>		
...
function rezerwy($row,$row_case_ann){		  
	...
	$result .= '<script language="JavaScript1.2" src="Scripts/js_skok_announce.js"></script>
...
if (isset($change['rezerwy_rezerwy'])){
		...
		<table cellpadding="1" cellspacing="0" border="1" align="center" width=90%>
		  <tr>				
			...
			//(SELECT nazwa  FROM coris_skok_swiadczenia    WHERE coris_skok_swiadczenia.ID=ccr.ID_swiadczenie  ) As swiadczenie,
		/*	 	$query = "SELECT cer.*,
			...
			FROM coris_skok_rezerwy as cer LEFT JOIN coris_assistance_cases_expenses as cace ON cer.ID_expenses = cace.expense_id 			
			WHERE cer.case_id = '$case_id'					
			ORDER BY ID";		*/
		
 	$query = "SELECT ccr.*,				 	
				 coris_skok_lista_swiadczen.ID,
				 coris_skok_lista_swiadczen.kwota As s_kwota,
				 coris_skok_lista_swiadczen.currency_id As s_currency_id,
				 coris_skok_zakres_ubezpieczenia.nazwa As zakres_nazwa,
				 coris_skok_swiadczenia.nazwa,
				 cace.contrahent_id,			
			(SELECT short_name  FROM coris_contrahents   WHERE coris_contrahents.contrahent_id=cace.contrahent_id  ) As wykonawca,
			...
			FROM coris_skok_lista_swiadczen,coris_skok_swiadczenia,coris_skok_zakres_ubezpieczenia,
								coris_skok_rezerwy as ccr LEFT JOIN coris_assistance_cases_expenses as cace ON ccr.ID_expenses = cace.expense_id 			
			WHERE ccr.case_id = '$case_id'		
			AND coris_skok_lista_swiadczen.ID = 	ccr.ID_swiadczenie		
			AND coris_skok_swiadczenia.ID = coris_skok_lista_swiadczen.ID_swiadczenie

...
//	return ;
    ...
    if (isset($change['ch_claims_wyplaty']) && $case_id > 0  ) {
        ...
        if ($edit_form_action == 'invoice_add'  && $edit_form_action_val > 0){
            ...
            $query = "SELECT * FROM coris_vig_payment WHERE ID='$edit_form_action_val'";
            ...
            if (mysql_num_rows($ms) > 0 ){
                ...
                $tow = array(5  => 14553,7 =>  14835);
                $sources = array(5  => 4,7 =>  5);


                $insertSQL1 = "INSERT INTO coris_assistance_cases_expenses SET 
                          case_id='".$ID_case."',                            
  					        contrahent_id='".$tow[$case->getClient_id()]."', 
  					        amount='$amount',  					      
                ...
                $insertSQL = "INSERT INTO coris_finances_invoices_in SET 
                          case_id='".$ID_case."', 
                            ...
                            invoice_in_due_date=curdate(),
  					        ...
  					        type=1,
                            ID_claim_source='".$sources[$case->getClient_id()]."',
                ...
                if ($inv_id > 0 ){
                    $query = "UPDATE coris_vig_payment SET 	ID_invoice_in='$inv_id', status=1  WHERE ID='$payment_id'";
                    $ms = mysql_query($query);
		...
		if ($res[0]){			   	
			...
			/*if($zgloszenie_kwota_zaakceptowana > 0.00 || $zgloszenie_pozycja_status==4){							
				...
				$cl = new VIGClaim($claims_id,$case_id);
				$cl->setAnnounce_date($data_zgloszenia);
				$cl->setNote($zgloszenie_uwagi);
				...
				if (is_array($tmp)){
					foreach ( $tmp As   $key => $val ){									
						...
						if ($add_zgloszenie_pozycja_usun != 1 ){//&& $zgloszenie_kwota>0
								$cls = new VIGClaimDetails(0, $claims_id,1);
								$cls->setKwota_roszczenia($zgloszenie_kwota);
								$cls->setWaluta($zgloszenie_waluta);
								if ( $zgloszenie_waluta == 'PLN' ){
										$cls->setKwota_rezerwa($zgloszenie_kwota); 	
		...
		if ($res[0]){			   	
			...
			if ($edit_form_action=='decissions_do_poprawy'){						
				...
				if (is_array($decyzja_lista_pozycji)){
						...
						VIGDecision::createDecision($case_id,$lista);
					...
					VIGDecision::updateDecision($case_id,$decisions_id, $tekst1, $tekst2, $tekst3, $tekst4, $tekst5, $data_decyzji);
		...
		if ($res[0]){			   	
			...
			$row_case_ann =VIGCase::getCaseInfo($case_id);
			...
			if (count($update) > 0 ){
										$query = "UPDATE coris_vig_announce SET 
										".implode(', ', $update)."
			...
			if (count($update_roznice) > 0 ){
				VIGCase::rejestrujZmiany($case_id,$case_id,'Roszczenie','UPDATE',$update_roznice);
			}							
		}
	}
	echo $message;	
...
if ($row_case_settings['client_id'] == 7  || $row_case_settings['client_id'] == 5){
	...
	<br><br><br><br><br><div align="center"><b>Tylko dla spraw TU VIG</b></div>
	</div>
	';
...
if (isset($change['claims_zgloszenia'])){
			...
			';
			$lista = VIGCase::getClaims($case_id);		
...
if ($branch == 1 && $checkRezerwy ){			
		...
		<tr bgcolor="#AAAAAA"><td width="70%" align="right"><b>Rezerwa globalna:</b></td><td width="30%">
			...
			if (getValue('edit_form_action') == 'claims_edit' && getValue('edit_form_action_param') > 0  ){
				...
				$cl = new VIGClaim($edit_cl, $case_id);
...
if (isset($change['claims_decyzje'])){
			...
			function probny_wydruk(){
					...
					popup(\'AS_cases_vig_claims_print_test.php?lista=\'+pozycje,\'wydruk\',1000,800);
						
			}	
</script>
			';
			$lista = VIGCase::getDecisions($case_id);		
						$result .= '
		    <table cellpadding="1" cellspacing="0" border="1" align="center" width=90%>
		      <tr bgcolor="#999999">
			...
			foreach ($lista As $pozycja  ){
						  ...
						  ';
								...
								foreach ($lista_pozycji As $poz){
									$result .= '<tr>';
										...
										$cls = new VIGClaimDetails($poz['ID_claims_details']);
				...
				$lista = VIGCase::getClaimsDetailsLista($case_id,'status2=1');		
						$result .= '
		   			 <table cellpadding="1" cellspacing="0" border="1" align="center" width=90%>
		      				<tr bgcolor="#999999">
		      					<td width="5%" align="center">&nbsp;</td>		      								
		  ...
		  $lista = VIGCase::getDecisions($case_id);		
			...
			foreach ($lista As $pozycja  ){
				...
				$result .= '<tr bgcolor="#BBBBBB">';
					...
					$result .= '<td align="center">'.$cl->getStatusName().' '.($cl->getStatus()==1 ? '<a href="AS_cases_vig_claims_print.php?id='.$cl->getId().'" target="_blank"><img border="0" src="img/print.gif"></a>' : '' ).'</td>';													
					$result .= '<td align="center">'.$cl->getTypeName().'</td>';													
					$result .= '<td>'.$cl->getDate().'</td>';																														
					$result .= '<td align="right">'.print_currency($cl->getPayment_amount()).' '.$cl->getCurrency_id().'</td>';																									
					$result .= '<td>'.Application::getUserName($cl->getID_user()).'</td>';												
...
//	return $result;
	...
	if (isset($change['claims_wyplaty'])){
			 ...
			 $query = "SELECT * FROM coris_vig_payment WHERE ID_case= '$case_id'";
			 $mysql_result = mysql_query($query);		
						$result .= '
		    <table cellpadding="1" cellspacing="0" border="1" align="center" width=90%>
		      <tr bgcolor="#999999">
